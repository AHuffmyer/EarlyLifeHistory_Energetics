...
```{r}
carbmetab<-c("Glucose", "Sucrose", "Fructose")

carbmetab_metab<-metab_expr[rownames(metab_expr) %in% carbmetab, ]
carbmetab_metab

carbmetab_metab$Function<-c("Carbohydrate Metabolism")
carbmetab_metab$metabolite_id<-rownames(carbmetab_metab)
rownames(carbmetab_metab) <- NULL
```

Create list of expression of genes for signalling: 

```{r}
signals<-c("Inositol")

signal_metab<-metab_expr[rownames(metab_expr) %in% signals, ]
signal_metab

signal_metab$Function<-c("Signalling")
signal_metab$metabolite_id<-rownames(signal_metab)
rownames(signal_metab) <- NULL
```

Create list of expression of genes for lipid membranes: 

```{r}
lipid<-c("Betaine")

lipid_metab<-metab_expr[rownames(metab_expr) %in% lipid, ]
lipid_metab

lipid_metab$Function<-c("Lipid Membrane")
lipid_metab$metabolite_id<-rownames(lipid_metab)
rownames(lipid_metab) <- NULL
```

Create list of expression of genes for nitrogen: 

```{r}
nitrogen <-c("Glutamate", "Arginine", "Ornithine", "Isoleucine")

nitrogen_metab<-metab_expr[rownames(metab_expr) %in% nitrogen, ]
nitrogen_metab

nitrogen_metab$Function<-c("Nitrogen")
nitrogen_metab$metabolite_id<-rownames(nitrogen_metab)
rownames(nitrogen_metab) <- NULL
```

Create list of expression of genes for antioxidants: 

```{r}
antiox <-c("Glutamine")

antiox_metab<-metab_expr[rownames(metab_expr) %in% antiox, ]
antiox_metab

antiox_metab$Function<-c("Antioxidant")
antiox_metab$metabolite_id<-rownames(antiox_metab)
rownames(antiox_metab) <- NULL
```

Create list of expression of genes for fatty acid metabolism: 

```{r}
fattyacid <-c("Glycine", "Glycerol-3-phosphate")

fattyacid_metab<-metab_expr[rownames(metab_expr) %in% fattyacid, ]
fattyacid_metab

fattyacid_metab$Function<-c("Fatty Acid")
fattyacid_metab$metabolite_id<-rownames(fattyacid_metab)
rownames(fattyacid_metab) <- NULL
```

Combine all lists together.  
```{r}
specific_metab<-rbind(antiox_metab, carbmetab_metab, fattyacid_metab, lipid_metab, nitrogen_metab, signal_metab)
```
...
carbon_plot<-specific_metab%>%
    group_by(Function, metabolite_id, group)%>%
    ...
    ggplot(., aes(x = group, y = mean, linetype=metabolite_id, group=metabolite_id)) +
    facet_wrap(~Function)+
    geom_line(aes(group=metabolite_id), color="black", position = position_dodge(0.1)) +
...
Plot signalling metabolites.  
```{r}
signal_plot<-specific_metab%>%
    group_by(Function, metabolite_id, group)%>%
    summarise(mean=mean(normalized_ions), N=length(normalized_ions), sd=sd(normalized_ions), sem=sd/N)%>%
    filter(Function=="Signalling")%>%
    ggplot(., aes(x = group, y = mean, linetype=metabolite_id, group=metabolite_id)) +
    facet_wrap(~Function)+
    geom_line(aes(group=metabolite_id), color="black", position = position_dodge(0.1)) +
    geom_point(aes(group=metabolite_id), size=2, position = position_dodge(0.1)) + 
...
Plot fatty acid metabolites.  
```{r}
fatty_plot<-specific_metab%>%
    group_by(Function, metabolite_id, group)%>%
    summarise(mean=mean(normalized_ions), N=length(normalized_ions), sd=sd(normalized_ions), sem=sd/N)%>%
    filter(Function=="Fatty Acid")%>%
    filter(!metabolite_id=="Glycerol-3-phosphate")%>%
    ggplot(., aes(x = group, y = mean, linetype=metabolite_id, group=metabolite_id)) +
    facet_wrap(~Function)+
    geom_line(aes(group=metabolite_id), color="black", position = position_dodge(0.1)) +
    geom_point(aes(group=metabolite_id), size=2, position = position_dodge(0.1)) + 
...
nitrogen_plot<-specific_metab%>%
    group_by(Function, metabolite_id, group)%>%
    summarise(mean=mean(normalized_ions), N=length(normalized_ions), sd=sd(normalized_ions), sem=sd/N)%>%
    filter(Function=="Nitrogen")%>%
    filter(!metabolite_id=="Isoleucine")%>%
    ggplot(., aes(x = group, y = mean, linetype=metabolite_id, group=metabolite_id)) +
    facet_wrap(~Function)+
    geom_line(aes(group=metabolite_id), color="black", position = position_dodge(0.1)) +
    geom_point(aes(group=metabolite_id), size=2, position = position_dodge(0.1)) + 
...
Plot lipid membrane metabolites.  
```{r}
betaine_plot<-specific_metab%>%
    group_by(Function, metabolite_id, group)%>%
    summarise(mean=mean(normalized_ions), N=length(normalized_ions), sd=sd(normalized_ions), sem=sd/N)%>%
    filter(Function=="Lipid Membrane")%>%
    ggplot(., aes(x = group, y = mean, linetype=metabolite_id, group=metabolite_id)) +
    facet_wrap(~Function)+
    geom_line(aes(group=metabolite_id), color="black", position = position_dodge(0.1)) +
    geom_point(aes(group=metabolite_id), size=2, position = position_dodge(0.1)) + 
...
Plot antioxidant metabolites.  
```{r}
antiox_plot<-specific_metab%>%
    group_by(Function, metabolite_id, group)%>%
    summarise(mean=mean(normalized_ions), N=length(normalized_ions), sd=sd(normalized_ions), sem=sd/N)%>%
    filter(Function=="Antioxidant")%>%
    ggplot(., aes(x = group, y = mean, linetype=metabolite_id, group=metabolite_id)) +
    facet_wrap(~Function)+
    geom_line(aes(group=metabolite_id), color="black", position = position_dodge(0.1)) +
    geom_point(aes(group=metabolite_id), size=2, position = position_dodge(0.1)) + 
...
metab_plots<-plot_grid(carbon_plot, antiox_plot, fatty_plot, nitrogen_plot, ncol=2, nrow=2, align="h")
                      
ggsave(filename="Mcap2020/Figures/Metabolomics/metabolite_expression_class.pdf", plot=metab_plots, dpi=300, width=10, height=8, units="in")
```