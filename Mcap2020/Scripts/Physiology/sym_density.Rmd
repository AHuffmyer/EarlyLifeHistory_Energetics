...
df%>%
  ...
  kbl(caption="Table X. Descriptive statistics of Symbiodiniaceae cell densities across ontogeny")%>%
  kable_classic(full_width=FALSE, html_font="Arial")%>%
  row_spec(0, bold = TRUE) %>%
  save_kable(.,"Mcap2020/Figures/Physiology/cell_density_table.png", zoom=3)
...
symb_plot<-df %>%
    ...
    scale_fill_brewer(name="Lifestage", palette="BrBG")+
...
symb_plot2<-df %>%
    ggplot(., aes(x = as.factor(hpf), y = cells.ind, fill=group)) +
    geom_boxplot(outlier.size = 0) +
    #geom_smooth(method="loess", se=TRUE, fullrange=TRUE, level=0.95, color="black") +
    geom_point(pch = 21, size=4, position = position_jitterdodge(0.2)) + 
    xlab("Hours Post-Fertilization)") + 
    scale_fill_brewer(name="Lifestage", palette="BrBG")+
    ylab(expression(bold(paste("Symbiont cells individual"^-1))))+
    ylim(0,9000)+
...
output.anova.table%>%
  kbl(caption="Table X. Linear mixed effect model analysis of Symbiodiniaceae cell densities across ontogeny", format="html", digits = c(3, 0, 0, 2, 11))%>%
  kable_classic(full_width=FALSE, html_font="Arial")%>%
  save_kable(.,"Mcap2020/Figures/Physiology/cell_density_model.png", zoom=3)
```
