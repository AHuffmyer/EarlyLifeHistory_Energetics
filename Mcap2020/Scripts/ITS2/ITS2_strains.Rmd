...
View pcoa by OTU.  
```{r}
GP.ordb <- ordinate(GP1, "PCoA", "bray")
p1b = plot_ordination(GP1, GP.ordb, type="taxa", title="taxa")
p1b
```

Plot with samples now.  
```{r}
p2b = plot_ordination(GP1, GP.ordb, type="samples", color="lifestage") 
...
perm1<-adonis2(phyloseq::distance(GP1, method="bray") ~ lifestage,
       data = metadata)

perm1
```

Permanova suggests there is separation by lifestage in ITS2 profiles (p=0.001).  
...
Pretty similar across life stages, maybe a trend for decrease across lifestages.     
...
There are 12 members of the community that shift by lifestage, 10 in clade C and 2 in clade D.  
