...
Before filtering, we had approximately 39,000 genes. After filtering for pOverA, we have approximately 15,000 genes. This indicates that there were many genes present in <7% of samples at <5 counts per gene.  
...
```{r}
allgenesfilt_PCA_visual <- ggplot(gPCAdata, aes(PC1, PC2, shape=lifestage, colour=lifestage)) + 
  geom_point(size=5) +
  xlab(paste0("PC1: ",percentVar[1],"% variance")) +
  ylab(paste0("PC2: ",percentVar[2],"% variance")) +
  scale_shape_manual(values = c("1_Egg"=17, "10_CalcifyingRecruit2"=16, "2_Embryo"=16, "3_Larvae1"=15, "4_Larvae2"=17, "5_Larvae3"=15, "6_Larvae4"=16, "7_Larvae6"=17, "8_MetamorposedRecruit2"=16, "9_CalcifyingRecruit1"=15), labels=c("Egg (1 hpf)", "Attached Recruit (231 hpf)", "Embryo (5 hpf)", "Embryo (38 hpf)", "Embryo (65 hpf)", "Larvae (93 hpf)", "Larvae (163 hpf)", "Larvae (231 hpf)", "Metamorphosed Recruit (231 hpf)", "Attached Recruit (183 hpf)")) +
  scale_colour_manual(values = c("1_Egg"="#8C510A", "10_CalcifyingRecruit2"="#BA55D3", "2_Embryo"="#DFC27D", "3_Larvae1"="#DFC27D", "4_Larvae2"="#DFC27D", "5_Larvae3"="#80CDC1", "6_Larvae4"="#80CDC1", "7_Larvae6"="#80CDC1", "8_MetamorposedRecruit2"="#003C30", "9_CalcifyingRecruit1"="#BA55D3"), labels=c("Egg (1 hpf)", "Attached Recruit (231 hpf)", "Embryo (5 hpf)", "Embryo (38 hpf)", "Embryo (65 hpf)", "Larvae (93 hpf)", "Larvae (163 hpf)", "Larvae (231 hpf)", "Metamorphosed Recruit (231 hpf)", "Attached Recruit (183 hpf)")) +
  #xlim(-40,40)+ 
  #ylim(-40,40)+
  coord_fixed()+
...
We have 12 modules with no modules "unassigned". All genes were assigned to a module.  
...
Currently 700+ are missing 

CHECK THIS TO SEE IF FORMATTING IS DIFFERENT????? WHY ARE 500 MISSING???

WE NEED TO REDO THE ANNOTATION

There are 771 gene_id in the datExpr that are not matched in the annotation file. Manually searching for these genes shows that they are not in the annotation file, which confirms that we do not have a gene_id formatting error. Unknown why some genes are not matched in the annotation file, look into this.    
...
Create list of expression of genes for carbonic anhydrase: 
...
Create list of expression of genes for glycogen synthase: 
...
Create list of expression of genes for sugar transporters: 

```{r}
sugartr<-c("g24450", "g62860", "g12293")
sugartr_genes<-gene_expr[rownames(gene_expr) %in% sugartr, ]
sugartr_genes

sugartr_genes$Function<-c("Sugar Transporter")
sugartr_genes$gene_id<-rownames(sugartr_genes)
rownames(sugartr_genes) <- NULL
```

Create list of expression of genes for ammonium transporters: 
...
Create list of expression of genes for acetyl CoA carboxylase: 

```{r}
acetylcarb<-c("g4402", "g52786")
acetylcarb_genes<-gene_expr[rownames(gene_expr) %in% acetylcarb, ]
acetylcarb_genes

acetylcarb_genes$Function<-c("Acetyl CoA Carboxylase")
acetylcarb_genes$gene_id<-rownames(acetylcarb_genes)
rownames(acetylcarb_genes) <- NULL
```

Create list of expression of genes for glutathione peroxidase activity: 
...
Create list of expression of genes for glucose transporters: 
...
Create list of expression of genes for solute transporters: 

```{r}
solutetr<-c("g68935", "adi2mcaRNA15205_R0", "adi2mcaRNA5856_R0", "adi2mcaRNA9950_R0", "g14988", "g15720", "g22279")
solutetr_genes<-gene_expr[rownames(gene_expr) %in% solutetr, ]
solutetr_genes

solutetr_genes$Function<-c("Solute Transporter")
solutetr_genes$gene_id<-rownames(solutetr_genes)
rownames(solutetr_genes) <- NULL
```

Create list of expression of genes for H+ATPase: 
...
Create list of expression of genes for glucose homeostasis: 
...
Create list of expression of genes for apoptosis regulation: 
...
Create list of expression of genes for cellular communication/signaling: 
...
Create list of expression of genes for carbohydrate metabolism: 
...
carbs_genes$Function<-c("Carbohydrate Metabolism")
carbs_genes$gene_id<-rownames(carbs_genes)
rownames(carbs_genes) <- NULL
```

Bring all of the data frames together.  
carbanhyd_genes
glycsyn_genes
sugartr_genes
ammontr_genes
...
```{r}
specific_genes<-rbind(carbanhyd_genes, glycsyn_genes, sugartr_genes, ammontr_genes, acetylcarb_genes, glutperox_genes, solutetr_genes, atpase_genes, glucosehomeo_genes, apoptosis_genes, communication_genes, carbs_genes)
```
...
```{r}
group_order<-c("Egg", "Embryo", "Larvae", "Metamorphosed Recruit", "Attached Recruit")
...
atpase_plot<-specific_genes%>%
    group_by(Function, gene_id, group)%>%
    summarise(mean=mean(vst), N=length(vst), sd=sd(vst), sem=sd/N)%>%
    filter(Function=="H+ ATPase")%>%
    ggplot(., aes(x = group, y = mean, linetype=gene_id, group=gene_id)) +
    facet_wrap(~Function)+
    geom_line(aes(group=gene_id), color="black", position = position_dodge(0.1)) +
    geom_point(aes(group=gene_id), size=2, position = position_dodge(0.1)) + 
  geom_errorbar(aes(group=gene_id, ymin=mean-sem, ymax=mean+sem), width=0.1, linetype="solid", color="black", position=position_dodge(0.1), size=0.5) + 
    xlab("Lifestage") + 
    ylab("VST expression")+
...
Plot Acetyl CoA Carboxylase

```{r}
acetyl_plot<-specific_genes%>%
    group_by(Function, gene_id, group)%>%
    summarise(mean=mean(vst), N=length(vst), sd=sd(vst), sem=sd/N)%>%
    filter(Function=="Acetyl CoA Carboxylase")%>%
    ggplot(., aes(x = group, y = mean, linetype=gene_id, group=gene_id)) +
    facet_wrap(~Function)+
    geom_line(aes(group=gene_id), color="black", position = position_dodge(0.1)) +
    geom_point(aes(group=gene_id), size=2, position = position_dodge(0.1)) + 
...
ammonium_plot<-specific_genes%>%
    ...
    #filter(gene_id=="g26425")%>%
    ggplot(., aes(x = group, y = mean, group=gene_id, linetype=gene_id)) +
    facet_wrap(~Function)+
    geom_line(aes(group=gene_id), color="black", position = position_dodge(0.1)) +
    geom_point(aes(group=gene_id), size=2, position = position_dodge(0.1)) + 
  geom_errorbar(aes(group=gene_id, ymin=mean-sem, ymax=mean+sem), width=0.1, linetype="solid", color="black", position=position_dodge(0.1), size=0.5) + 
...
Plot Solute Transporter

```{r}
solute_plot<-specific_genes%>%
    group_by(Function, gene_id, group)%>%
    summarise(mean=mean(vst), N=length(vst), sd=sd(vst), sem=sd/N)%>%
    filter(Function=="Solute Transporter")%>%
    ggplot(., aes(x = group, y = mean, linetype=gene_id, group=gene_id)) +
    facet_wrap(~Function)+
    geom_line(aes(group=gene_id), color="black", position = position_dodge(0.1)) +
    geom_point(aes(group=gene_id), size=2, position = position_dodge(0.1)) + 
...
Plot Sugar Transporter

```{r}
sugar_plot<-specific_genes%>%
    group_by(Function, gene_id, group)%>%
    summarise(mean=mean(vst), N=length(vst), sd=sd(vst), sem=sd/N)%>%
    filter(Function=="Sugar Transporter")%>%
    ggplot(., aes(x = group, y = mean, linetype=gene_id, group=gene_id)) +
    facet_wrap(~Function)+
    geom_line(aes(group=gene_id), color="black", position = position_dodge(0.1)) +
    geom_point(aes(group=gene_id), size=2, position = position_dodge(0.1)) + 
...
gene_plots<-plot_grid(anhydrase_plot, acetyl_plot, ammonium_plot, atpase_plot, glucose_plot, glutathione_plot, glycogen_plot, solute_plot, sugar_plot, apoptosis_plot, carb_plot, ncol=6, nrow=2, align="h")
                      
ggsave(filename="Mcap2020/Figures/TagSeq/gene_expression_class.pdf", plot=gene_plots, dpi=300, width=15, height=8, units="in")
```
