...
trends<-data_summary%>%
  spread(Lifestage, mean)%>%
  mutate(Trend=ifelse(Larvae1>Larvae2 & Larvae2>Recruit1, "Decreasing", 
                      ifelse(Larvae1<Larvae2 & Larvae2<Recruit1, "Increasing", "Variable")))
```
...
MyResult.splsda.final <- splsda(X, Y, ncomp = 2, keepX = c(30,30)) #placeholder numbers for now until validation works
...
Select the stable features in each component.  
```{r}
