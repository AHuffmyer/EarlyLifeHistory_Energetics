...
There are 11,475 genes in our Gene Info file from genes detected in our dataset.   
...
Run a loop to plot enrichment for each module at each level of ontology. Use only levels with enriched GO terms. Modules 12, 9, 6 and 5 had no enriched terms and 7 had too few.  

```{r}
#module_vector<-c("1", "2", "3", "7", "8")
#module_vector<-c("7", "8")
module_vector<-c("8")
ontologies<-c("BP", "MF")
...
ggsave(filename=paste0("Mcap2020/Figures/TagSeq/GenomeV3/GOSeq/GOenrich_Module", module, "_", category, ".png"), plot=GO.plot, dpi=100, width=12, height=40, units="in", limitsize=FALSE)
...
```{r}
#pattern_vector<-c("Increasing", "Decreasing", "Mid")
pattern_vector<-c("Decreasing", "Mid")
ontologies<-c("BP", "MF")
...
## 4. Plot enrichment for each ontogenetic pattern     

Plot enrichment for each pattern together in one plot. Read in files.  
```{r}
...
Plot all together for increasing. 
```{r}
#add vector for terms of interest to reduce number of GO terms 
    #Read relevant file of results from goseq analysis  
    ...
    #plot significantly enriched GO terms by Slim Category faceted by slim term 
  GO.plot.increasing <-  go_results%>%
    filter(pattern=="Increasing")%>%
    mutate(percHits=(numDEInCat/numInCat)*100)%>%
  
    ggplot(aes(x = percHits, y = term, colour=bh_adjust)) + 
    geom_point(aes(size=percHits, fill=bh_adjust, colour=bh_adjust)) + 
    scale_size(name="% Hits", range=c(3,6))+
    ...
    xlab("Hits (%)")+
    ggtitle("Increasing across development")+
   ...
   scale_fill_gradientn(colours = colorspace::heat_hcl(7))+
    ...
    theme(panel.border = element_blank(), 
          ...
          axis.title.y = element_blank());GO.plot.increasing

ggsave(filename=paste0("Mcap2020/Figures/TagSeq/GenomeV3/GOSeq/GOenrich_Increasing_REVISED.jpeg"), plot=GO.plot.increasing, dpi=300, width=12, height=12, units="in")
ggsave(filename=paste0("Mcap2020/Figures/TagSeq/GenomeV3/GOSeq/GOenrich_Increasing_REVISED.pdf"), plot=GO.plot.increasing, dpi=300, width=12, height=12, units="in")
```

Plot all together for mid development. 
```{r}
    #plot significantly enriched GO terms by Slim Category faceted by slim term 
  GO.plot.mid <-  go_results%>%
    filter(pattern=="Mid")%>%
    mutate(percHits=(numDEInCat/numInCat)*100)%>%
    ...
    facet_grid(ParentTerm ~ ., scales = "free", space='free', labeller = label_wrap_gen(width = 6, multi_line = TRUE))+
    xlab("Hits (%)")+
    ggtitle("Peaking in mid-development")+
    geom_segment(aes(x=0, xend=percHits, y=term, yend=term),linewidth=1) +
   scale_colour_gradientn(colours = colorspace::heat_hcl(7))+
   scale_fill_gradientn(colours = colorspace::heat_hcl(7))+
    theme_classic() + 
    theme(panel.border = element_blank(), 
          ...
          title=element_text(size=16, face="bold"),
          plot.title = element_text(),
          axis.title.y = element_blank());GO.plot.mid

ggsave(filename=paste0("Mcap2020/Figures/TagSeq/GenomeV3/GOSeq/GOenrich_Mid_REVISED.jpeg"), plot=GO.plot.mid, dpi=300, width=16, height=40, units="in")
ggsave(filename=paste0("Mcap2020/Figures/TagSeq/GenomeV3/GOSeq/GOenrich_Mid_REVISED.pdf"), plot=GO.plot.mid, dpi=300, width=16, height=40, units="in")
```


Plot all together for decreasing development. 
```{r}
    #plot significantly enriched GO terms by Slim Category faceted by slim term 
  GO.plot.decreasing <-  go_results%>%
    filter(pattern=="Decreasing")%>%
    mutate(percHits=(numDEInCat/numInCat)*100)%>%
    ...
    facet_grid(ParentTerm ~ ., scales = "free", space='free', labeller = label_wrap_gen(width = 6, multi_line = TRUE))+
    xlab("Hits (%)")+
    ggtitle("Decreasing across development")+
    geom_segment(aes(x=0, xend=percHits, y=term, yend=term),linewidth=1) +
   scale_colour_gradientn(colours = colorspace::heat_hcl(7))+
   scale_fill_gradientn(colours = colorspace::heat_hcl(7))+
    theme_classic() + 
    theme(panel.border = element_blank(), 
          ...
          title=element_text(size=16, face="bold"),
          plot.title = element_text(),
          axis.title.y = element_blank());GO.plot.decreasing

ggsave(filename=paste0("Mcap2020/Figures/TagSeq/GenomeV3/GOSeq/GOenrich_Decreasing_REVISED.jpeg"), plot=GO.plot.decreasing, dpi=300, width=16, height=80, units="in", limitsize=FALSE)
ggsave(filename=paste0("Mcap2020/Figures/TagSeq/GenomeV3/GOSeq/GOenrich_Decreasing_REVISED.pdf"), plot=GO.plot.decreasing, dpi=300, width=16, height=80, units="in", limitsize=FALSE)
```



NEXT
- Make gene tables and look at gene lists 
- Frame patterns as time periods of HPF rather than patterns 
- Make final figures of interest 
...
Customize first for carbohydrate metabolism related BP ontology. 
```{r}
...
ggsave(filename=paste0("Mcap2020/Figures/TagSeq/GenomeV3/GOSeq/GOenrich_AllPatterns_Carbohdyrate.pdf"), plot=GO.plot.carb, dpi=300, width=13, height=18, units="in")
...
ggsave(filename=paste0("Mcap2020/Figures/TagSeq/GenomeV3/GOSeq/GOenrich_AllPatterns_Lipids.pdf"), plot=GO.plot.lipid, dpi=300, width=13, height=18, units="in")
...
#add vector for terms of interest to reduce number of GO terms 
symbiosis<-c("regulation of cell communication", "transmembrane transport", "transmembrane", "organic substance transport", "antioxidant", "oxygen", "involved in interspecies interaction")
...
ggsave(filename=paste0("Mcap2020/Figures/TagSeq/GenomeV3/GOSeq/GOenrich_AllPatterns_Symbiosis.png"), plot=GO.plot.sym, dpi=300, width=14, height=18, units="in")
ggsave(filename=paste0("Mcap2020/Figures/TagSeq/GenomeV3/GOSeq/GOenrich_AllPatterns_Symbiosis.pdf"), plot=GO.plot.sym, dpi=300, width=14, height=18, units="in")
...
For fun, look at functions related to epigenetics and methylation for E5 meeting discussion. 
```{r}
category<-c("BP")

#add vector for terms of interest to reduce number of GO terms 
epigen<-c("epigenetic", "methylation", "methyl", "histone", "chromatin", "transferase")
    ...
    go_results<-go_results%>%
      filter(grepl(paste(epigen, collapse="|"), ParentTerm))
      
    #plot significantly enriched GO terms by Slim Category faceted by slim term 
  GO.plot.epi <-  ggplot(go_results, aes(x = pattern, y = term)) + 
    geom_point(aes(size=numInCat, fill=bh_adjust, colour=bh_adjust)) + 
    scale_size(name="Number of Genes", range=c(5,12), limits=c(1,550))+
    scale_fill_gradient(name = "Adjusted p-value", low="black", high="gray")+
    scale_colour_gradient(name = "Adjusted p-value", low="black", high="gray")+
...
ggsave(filename=paste0("Mcap2020/Figures/TagSeq/GenomeV3/GOSeq/GOenrich_AllPatterns_Epigenetics.png"), plot=GO.plot.epi, dpi=300, width=14, height=14, units="in")
ggsave(filename=paste0("Mcap2020/Figures/TagSeq/GenomeV3/GOSeq/GOenrich_AllPatterns_Epigenetics.pdf"), plot=GO.plot.epi, dpi=300, width=14, height=14, units="in")

#save object for merging all plots together for manuscript
go_results_epi<-go_results%>%
  mutate(category="Epigenetics")

```

Look at functions related to nitrogen metabolism.  
```{r}
category<-c("BP")

#add vector for terms of interest to reduce number of GO terms 
nitrogen<-c("nitrogen", "ammonium", "nitrification", "nitrate", "nitrite")
    ...
    go_results<-go_results%>%
      filter(grepl(paste(nitrogen, collapse="|"), ParentTerm))
      
    #plot significantly enriched GO terms by Slim Category faceted by slim term 
  GO.plot.nitrogen <-  ggplot(go_results, aes(x = pattern, y = term)) + 
    geom_point(aes(size=numInCat, fill=bh_adjust, colour=bh_adjust)) + 
    scale_size(name="Number of Genes", range=c(5,12), limits=c(1,550))+
    scale_fill_gradient(name = "Adjusted p-value", low="black", high="gray")+
    scale_colour_gradient(name = "Adjusted p-value", low="black", high="gray")+
...
ggsave(filename=paste0("Mcap2020/Figures/TagSeq/GenomeV3/GOSeq/GOenrich_AllPatterns_Nitrogen.png"), plot=GO.plot.nitrogen, dpi=300, width=14, height=28, units="in")
ggsave(filename=paste0("Mcap2020/Figures/TagSeq/GenomeV3/GOSeq/GOenrich_AllPatterns_Nitrogen.pdf"), plot=GO.plot.nitrogen, dpi=300, width=14, height=28, units="in")

#save object for merging all plots together for manuscript
go_results_calc<-go_results%>%
  mutate(category="Nitrogen")

```

For fun, look at functions related to cell death. 
```{r}
category<-c("BP")

#add vector for terms of interest to reduce number of GO terms 
death<-c("death", "apoptosis")
    ...
    go_results<-go_results%>%
      filter(grepl(paste(death, collapse="|"), ParentTerm))
      
    #plot significantly enriched GO terms by Slim Category faceted by slim term 
  GO.plot.death <-  ggplot(go_results, aes(x = pattern, y = term)) + 
    geom_point(aes(size=numInCat, fill=bh_adjust, colour=bh_adjust)) + 
    scale_size(name="Number of Genes", range=c(5,12), limits=c(1,550))+
    scale_fill_gradient(name = "Adjusted p-value", low="black", high="gray")+
    scale_colour_gradient(name = "Adjusted p-value", low="black", high="gray")+
...
ggsave(filename=paste0("Mcap2020/Figures/TagSeq/GenomeV3/GOSeq/GOenrich_AllPatterns_Death.png"), plot=GO.plot.death, dpi=300, width=14, height=14, units="in")
ggsave(filename=paste0("Mcap2020/Figures/TagSeq/GenomeV3/GOSeq/GOenrich_AllPatterns_Death.pdf"), plot=GO.plot.death, dpi=300, width=14, height=14, units="in")

#save object for merging all plots together for manuscript
go_results_epi<-go_results%>%
  mutate(category="Death")
...
ggsave(filename="Mcap2020/Figures/TagSeq/GenomeV3/ontogeny_patterns_expression.png", plot=pattern_plots, dpi=50 , width=16, height=8, units="in")
ggsave(filename="Mcap2020/Figures/TagSeq/GenomeV3/ontogeny_patterns_expression.pdf", plot=pattern_plots, dpi=50 , width=16, height=8, units="in")
```
...
pattern_expression$lifestage<-factor(pattern_expression$lifestage, levels=time_point_order)

genes_rel_change<-pattern_expression%>%
  dplyr::select(lifestage, sample_id, Pattern, Mean)%>%
  filter(!Pattern=="NA")%>%
  group_by(lifestage, Pattern)%>%
  summarise(mean=mean(Mean))%>% #calculate means
  group_by(lifestage, Pattern)%>%
  arrange(lifestage, .by_group = TRUE)%>%
  group_by(Pattern)%>%
  mutate(gene_expression=((mean-dplyr::first(mean))/abs(dplyr::first(mean))))%>%
  dplyr::select(Pattern, lifestage, gene_expression)%>%
  write_csv("Mcap2020/Data/Integration/RelativeChange/genes_relchange.csv")
```
