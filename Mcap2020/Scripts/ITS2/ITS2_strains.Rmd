...
In rarefication 15 OTUs were removed because they were no longer present in any sample after random subsampling. 
...
View pcoa by OTU.  
```{r}
GP.ordb <- ordinate(GP1_rare, "NMDS", "bray")
...
Permanova suggests there is no significant separation by lifestage in ITS2 profiles (p=0.217).  
...
lifestage  9   0.3599 0.2609 1.0982  0.217
Residual  28   1.0195 0.7391              
Total     37   1.3794 1.0000    
...
There were 63 OTU's before filtering and 13 after filtering.  
```{r}
GP1_rare

GP1_prune = filter_taxa(GP1_rare, function(x) mean(x) > 1, TRUE)
GP1_prune 
...
pdf(file = "Mcap2020/Figures/ITS2/DIV-ComplexHeatmap.pdf", height = 8, width = 8)
ComplexHeatmap::Heatmap(table, name = "Rel. Abun. (%)", row_title = "DIV", column_title = "ITS2 DIV Relative Abundance (>1%)", 
        col = col_fun, 
        row_names_side = "left", 
        row_dend_side = "left",
        width = unit(4, "in"), 
...
```{r}
GP2 <- prune_taxa(taxa_sums(GP1_rare) > 0, GP1_rare)
```
...
alpha1_plot <- alpha1 + geom_point(size=3) + theme_classic() + scale_color_manual() + scale_color_manual(name="Lifestage", 
...
There are no changes in alpha diversity of ITS2 in development.  

Export relative change in alpha diversity for integrative analyses. Calculate relative change for all lifestages compared to the first time point in this dataset (eggs). We will use the Shannon Index for this calculation.    
...
alpha_rel_change<-alpha_rel_change%>%
  arrange(lifestage)%>%
  mutate(symbiont_alpha_diversity=((mean_alpha-dplyr::first(.$mean_alpha))/dplyr::first(.$mean_alpha)))%>%
  select(lifestage, symbiont_alpha_diversity)%>%
  write_csv("Mcap2020/Data/Integration/RelativeChange/ITS2_alpha_relchange.csv")
```
...
t_plot<-t + scale_fill_manual(name="ITS2 Type Profile", values=c("darkorange", "brown4")) + geom_hline(yintercept=50, linetype="dashed", color = "black", size=1) + facet_grid(~lifestage, scales = "free", space = "free", labeller = label_wrap_gen(width=10)) + ylab("Relative Abundance (%)") + theme(strip.text.x = element_text(size = 5, face="bold"))
