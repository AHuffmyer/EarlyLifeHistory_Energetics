...
For fun, look at functions related to calcification. 
```{r}
category<-c("BP")

#add vector for terms of interest to reduce number of GO terms 
calcification<-c("pH", "calcium", "calcification", "skeleton", "cation", "mineral")
    ...
    go_results<-go_results%>%
      filter(grepl(paste(calcification, collapse="|"), ParentTerm))
      
    #plot significantly enriched GO terms by Slim Category faceted by slim term 
  GO.plot.calc <-  ggplot(go_results, aes(x = pattern, y = term)) + 
    geom_point(aes(size=numInCat, fill=bh_adjust, colour=bh_adjust)) + 
    scale_size(name="Number of Genes", range=c(5,12), limits=c(1,550))+
    scale_fill_gradient(name = "Adjusted p-value", low="black", high="gray")+
    scale_colour_gradient(name = "Adjusted p-value", low="black", high="gray")+
...
ggsave(filename=paste0("Mcap2020/Figures/TagSeq/GenomeV3/GOSeq/GOenrich_AllPatterns_Calcification.png"), plot=GO.plot.calc, dpi=300, width=20, height=28, units="in")
ggsave(filename=paste0("Mcap2020/Figures/TagSeq/GenomeV3/GOSeq/GOenrich_AllPatterns_Calcification.pdf"), plot=GO.plot.calc, dpi=300, width=20, height=28, units="in")

#save object for merging all plots together for manuscript
go_results_calc<-go_results%>%
  mutate(category="Calcification")
