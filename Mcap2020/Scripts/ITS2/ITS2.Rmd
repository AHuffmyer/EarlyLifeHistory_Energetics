...
plot5<-data%>%
  ...
  group_by(group, hpf)%>%
  #summarise(mean=mean(relabund), sd=sd(relabund), N=length(relabund), se=sd/sqrt(N))%>%
  ggplot(., aes(x = as.factor(hpf), y = ratio)) +
    geom_point(aes(group=interaction(group), colour=group), size=6, alpha=1, position = position_dodge(0.7)) + 
...
ggsave(plot5, file="Mcap2020/Figures/ITS2/regression.png", width=8, height=6, dpi=300)
```
