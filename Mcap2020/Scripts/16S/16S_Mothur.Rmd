...
CHECK THIS BELOW TO SEE WHAT OUR SAMPLE SIZE IS FOR THESE!!! 
...
p2<-p2 + geom_polygon(aes(fill=lifestage)) + geom_point(size=5) + ggtitle("subsampled dataset")
p2
```
...
There is significant difference in the centroids between life stages in the rarefied dataset.  
...
Plot the two most commonly used - Shannon and Inverse Simpson.  
```{r}
plot_richness(GP2, measures=c("InvSimpson", "Shannon"), x="lifestage", color="group")
```
...
alpha_rare<-estimate_richness(GP2, measures=c("InvSimpson", "Shannon"))
...
There are significant differences in alpha diversity between lifestages in both the full and rare datasets.   
