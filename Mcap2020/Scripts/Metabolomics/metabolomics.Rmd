...
This script processes and prepares metabolomics data for analysis and conducts multivariate visualizations, supervised multivariate analyses, and differential metabolite analyses. 
...
write_csv(Timepoint_VIP_table, "Mcap2020/Output/Metabolomics/All_Metabolite_VIP_values.csv")
...
# Plot specific metabolite concentrations  

- Glucose
- Glutamine
- Glutamate
- ATP/ADP
- Total montiporic acids 





Glucose, glutamine, and glutamate 
```{r}
metabolites$HPF<-factor(metabolites$HPF, levels=c("Egg (1 hpf)", "Embryo (5 hpf)", "Embryo (38 hpf)", "Embryo (65 hpf)", "Larvae (93 hpf)", "Larvae (163 hpf)", "Larvae (183 hpf)", "Larvae (231 hpf)", "Metamorphosed Polyp (183 hpf)","Metamorphosed Polyp (231 hpf)", "Attached Recruit (183 hpf)", "Attached Recruit (231 hpf)", "Attached Recruit (255 hpf)"))

glucose_plot<-metabolites%>%
  filter(compound %in% c("Glucose"))%>%
  
  group_by(HPF, compound, Development)%>%
  summarise(mean=mean(log_norm_counts), se=sd(log_norm_counts)/sqrt(length(log_norm_counts)))%>%
  group_by(compound)%>%
  
  ggplot(aes(x=HPF, y=mean, fill=HPF))+
  geom_bar(stat="identity", colour="black")+
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width=0, colour="black")+
  scale_fill_manual(values=lifestage_cols)+ 
  theme_classic()+
  xlab("")+
  ylab("Pool Size")+
...
ggsave(glucose_plot, filename="Mcap2020/Figures/Metabolomics/VIPs/glucose.pdf", width=5, height=4)
```

```{r}
glutamine_plot<-metabolites%>%
...
ggsave(glutamine_plot, filename="Mcap2020/Figures/Metabolomics/VIPs/glutamine.pdf", width=5, height=4)
```

```{r}
glutamate_plot<-metabolites%>%
  filter(compound %in% c("Glutamate"))%>%
...
ggsave(glutamate_plot, filename="Mcap2020/Figures/Metabolomics/VIPs/glutamate.pdf", width=5, height=4)
```

```{r}
atp_plot<-metabolites%>%
  filter(compound %in% c("ATP"))%>%
...
ggsave(atp_plot, filename="Mcap2020/Figures/Metabolomics/VIPs/atp.pdf", width=5, height=4)
```

Total Montiporic acids
```{r}
montiporic_plot<-metabolites%>%
  filter(compound %in% c("Montiporic Acid A", "Montiporic Acid B", "Montiporic Acid C", "Montiporic Acid D"))%>%
  
  group_by(Lifestage, Rep, HPF)%>%
  summarise(sum=sum(log_norm_counts))%>%
  group_by(HPF)%>%
  summarise(mean=mean(sum), se=sd(sum)/sqrt(length(sum)))%>%
...
ggsave(montiporic_plot, filename="Mcap2020/Figures/Metabolomics/VIPs/acids.pdf", width=5, height=4)
```

```{r}
dipep_plot<-metabolites%>%
  filter(compound %in% c("Arginine-Glutamine"))%>%
...
ggsave(dipep_plot, filename="Mcap2020/Figures/Metabolomics/VIPs/arg-gln.pdf", width=5, height=4)
```

## ANOVA models for metabolites of interest 

Perform ANOVA for each metabolite of interest 
...
model<-aov(log_norm_counts ~ HPF, data=model_data)

summary(model)

qqPlot(residuals(model))
leveneTest(residuals(model)~model_data$HPF)

emm<-emmeans(model, ~HPF)
pairs(emm)
