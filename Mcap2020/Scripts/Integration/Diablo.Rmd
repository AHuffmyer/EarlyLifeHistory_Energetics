---
title: "Integration"
author: "AS Huffmyer"
output:
  html_document:
    toc: yes
    toc_depth: 6
    toc_float: yes
  pdf_document:
    keep_tex: yes
editor_options: 
  chunk_output_type: console
---

Run trial of Diablo scripts for genes and metabolomics. 

# **Setup**  

Set up workspace, set options, and load required packages.    
```{r}
knitr::opts_chunk$set(echo = TRUE, warning = FALSE, message = FALSE)
```

```{r setup, include = FALSE}
## install packages if you dont already have them
if (!require("tidyverse")) install.packages("tidyverse")
if (!require("ggplot2")) install.packages("ggplot2")
if (!require("janitor")) install.packages("janitor")
if (!require("mixOmics")) install.packages("mixOmics")
if (!require("genefilter")) install.packages("genefilter")
if (!require("DESeq2")) install.packages("DESeq2")

# load packages
library(ggplot2)
library(corpcor)
library(janitor)
library(mixOmics)
library(tidyverse)
library(genefilter)
library(DESeq2)

```

# **Metabolomics SPLSDA**  

We need to load data to have samples in rows and features in columns. We are not yet summarizing by lifestage, we will perform individual SPLSDA analyses on each full dataset to obtain keepX optimization.  

Metadata 
```{r}
metadata<-read.csv("Mcap2020/Data/lifestage_metadata.csv")

genes_metadata <- read.csv("Mcap2020/Data/TagSeq/Sample_Info.csv", header = TRUE, sep = ",")
head(genes_metadata)
```

Metabolomics
```{r}
metabolomics<-read.csv("Mcap2020/Data/Integration/selected_metabolites.csv")

metabolomics<-metabolomics%>%
  mutate(Sample=paste(Lifestage, Rep))%>%
  dplyr::select(Sample, Lifestage, compound, norm_counts)%>%
  spread(compound, norm_counts)

metabolomics<-as.data.frame(metabolomics)

```

Create PLSDA
```{r}
#assigning datasets 
X <- metabolomics[3:184]

Y <- as.factor(metabolomics$Lifestage) #select treatment names
Y

MyResult.plsda_metab <- plsda(X,Y, ncomp=9) #number of components is classes-1

#MyResult.splsda_metab <- splsda(X,Y, ncomp=12) #number of components is classes-1

plotIndiv(MyResult.plsda_metab) 
```

Run SPLSDA to tune parameters

Plot errors
```{r}

MyPerf.plsda_metab <- perf(MyResult.plsda_metab, validation = "Mfold", folds=4, 
                     progressBar = TRUE, nrepeat = 20) # we suggest nrepeat = 50

plot(MyPerf.plsda_metab,  sd = TRUE)
```

Determine keepX
```{r}
#now run tune splsda 
list.keepX <- c(2:10,  seq(20, 300, 10))

set.seed(30) # for reproducbility in this vignette, otherwise increase nrepeat
tune.splsda.srbct <- tune.splsda(X, Y, ncomp = 9, 
                                 validation = 'Mfold', folds=4,
                                 measure = "BER", test.keepX = list.keepX, nrepeat=20)   # we suggest nrepeat = 50

error <- tune.splsda.srbct$error.rate
ncomp.metab <- tune.splsda.srbct$choice.ncomp$ncomp # optimal number of components based on t-tests on the error rate
ncomp.metab

select.keepX.metab <- tune.splsda.srbct$choice.keepX# optimal number of variables to select
select.keepX.metab

plot(tune.splsda.srbct)
```

The optimal number of components is ncomp=8 with keepX as follows. 

comp1 comp2 comp3 comp4 comp5 comp6 comp7 comp8 comp9 
    2     3    10     5   140     2     3   110     8 

# **Gene SPLSDA**  

We need to load data to have samples in rows and features in columns. We are not yet summarizing by lifestage, we will perform individual SPLSDA analyses on each full dataset to obtain keepX optimization.  

Load data   
```{r}
genes<-read.csv("Mcap2020/Output/TagSeq/transformed_filtered_genes.csv") 

nrow(genes)
#15797 genes 

rownames(genes)<-genes$X

genes<-genes[-c(1)]

#transpose to have samples in rows and genes in columns
dim(genes)

genes_t<-t(genes)

dim(genes_t)

rownames(genes_t)
```


```{r}
#correct for lifestage names to match metabolomics
genes_t<-as.data.frame(genes_t)
dim(genes_t)

genes_t$Lifestage<-genes_metadata$code[match(rownames(genes_t), genes_metadata$sample_id)]
dim(genes_t)

#summarize gene counts for each lifestage
gene_data<-genes_t%>%
  relocate(Lifestage) #%>%
  #group_by(Lifestage)%>%
  #summarise_at(vars(2:15797), mean, na.rm = TRUE)

#finally make the lifestage the row names
gene_data<-as.data.frame(gene_data)

#rownames(gene_data)<-gene_data$Lifestage

dim(gene_data)

```

Create PLSDA
```{r}
#assigning datasets 
X <- gene_data[2:15798]

Y <- as.factor(gene_data$Lifestage) #select treatment names
Y

MyResult.plsda_gene <- plsda(X,Y, ncomp=9) #number of components is classes-1

plotIndiv(MyResult.plsda_gene) 
```

Run SPLSDA to tune parameters

Plot errors
```{r}

MyPerf.plsda_gene <- perf(MyResult.plsda_gene, validation = "Mfold", folds=3, #fold #? 
                     progressBar = TRUE, nrepeat = 10) # we suggest nrepeat = 50

plot(MyPerf.plsda_gene,  sd = TRUE)

#gene_data<-gene_data%>%
  #dplyr::select(!Lifestage)
```

Determine keepX
```{r}
#now run tune splsda 
list.keepX <- c(2:10,  seq(20, 300, 10))

set.seed(30) # for reproducbility in this vignette, otherwise increase nrepeat
tune.splsda.srbct_gene <- tune.splsda(X, Y, ncomp = 9, 
                                 validation = 'Mfold', folds=3,
                                 measure = "BER", test.keepX = list.keepX, nrepeat=20)   # we suggest nrepeat = 50

error <- tune.splsda.srbct_gene$error.rate
ncomp.gene <- tune.splsda.srbct_gene$choice.ncomp$ncomp # optimal number of components based on t-tests on the error rate
ncomp.gene

select.keepX.gene <- tune.splsda.srbct_gene$choice.keepX# optimal number of variables to select
select.keepX.gene

plot(tune.splsda.srbct_gene)
```

The optimal number of components is ncomp=8 and keepX as follows.  

comp1 comp2 comp3 comp4 comp5 comp6 comp7 comp8 comp9 
    2    10     2     3   300   300   140   270   220
    
    
# **ITS2 SPLSDA**  

We need to load data to have samples in rows and features in columns. We are not yet summarizing by lifestage, we will perform individual SPLSDA analyses on each full dataset to obtain keepX optimization.  

Load data   
```{r}
its2_metadata <- read.csv("Mcap2020/Data/ITS2/ITS2_Metadata.csv", header = TRUE, sep = ",")

its2<-read.csv("Mcap2020/Data/Integration/its2_div.csv") 

nrow(its2)
#63 symbiont DIVs 

rownames(its2)<-its2$X

its2<-its2[-c(1)]

#transpose to have samples in rows and genes in columns
dim(its2)

its2_t<-t(its2)

dim(its2_t)

rownames(its2_t)
```


```{r}
#correct for lifestage names to match metabolomics
its2_t<-as.data.frame(its2_t)
dim(its2_t)

its2_t$Lifestage<-its2_metadata$code[match(rownames(its2_t), its2_metadata$sample_name)]
dim(its2_t)

#summarize gene counts for each lifestage
its2_data<-its2_t%>%
  relocate(Lifestage) #%>%
  #group_by(Lifestage)%>%
  #summarise_at(vars(2:15797), mean, na.rm = TRUE)
head(its2_data)

its2_data<-as.data.frame(its2_data)

#rownames(its2_data)<-its2_data$Lifestage

#its2_data<-its2_data%>%
  #dplyr::select(!Lifestage)

dim(its2_data)

```

Create PLSDA
```{r}
#assigning datasets 
X <- its2_data[2:64]

Y <- as.factor(its2_data$Lifestage) #select treatment names
Y

MyResult.plsda_its2 <- plsda(X,Y, ncomp=9) #number of components is classes-1

plotIndiv(MyResult.plsda_its2) 
```

Run SPLSDA to tune parameters

Plot errors
```{r}

MyPerf.plsda_its2 <- perf(MyResult.plsda_its2, validation = "Mfold", folds=3, #fold #? 
                     progressBar = TRUE, nrepeat = 10) # we suggest nrepeat = 50

plot(MyPerf.plsda_its2,  sd = TRUE)
```

Determine keepX
```{r}
#now run tune splsda 
list.keepX <- c(2:10,  seq(20, 70, 10))

set.seed(30) # for reproducible in this vignette, otherwise increase nrepeat
tune.splsda.srbct_its2 <- tune.splsda(X, Y, ncomp = 9, 
                                 validation = 'Mfold', folds=3,
                                 measure = "BER", test.keepX = list.keepX, nrepeat=20)   

error <- tune.splsda.srbct_its2$error.rate
ncomp.its2 <- tune.splsda.srbct_its2$choice.ncomp$ncomp # optimal number of components based on t-tests on the error rate
ncomp.its2

select.keepX.its2 <- tune.splsda.srbct_its2$choice.keepX# optimal number of variables to select
select.keepX.its2

plot(tune.splsda.srbct_its2)
```

The optimal number of components is ncomp=9 and keepX as the following.  

comp1 comp2 comp3 comp4 comp5 comp6 comp7 comp8 comp9 
   30     7    60    10    20    20     7    20     7 
    
# **16S SPLSDA**  

We need to load data to have samples in rows and features in columns. We are not yet summarizing by lifestage, we will perform individual SPLSDA analyses on each full dataset to obtain keepX optimization.  

Load data   
```{r}
bacteria_metadata <- read.table("Mcap2020/Data/16S/mcap_metadata.txt", sep='\t', header=TRUE)

bacteria<-read.csv("Mcap2020/Data/Integration/16s_matrix.csv") 
rownames(bacteria)<-bacteria$taxa
bacteria<-bacteria%>%
  dplyr::select(!taxa)

bacteria_metadata<-bacteria_metadata%>%
  mutate(lifestage=if_else(lifestage=="Metamorphosed Recruit (183 hpf)", "Metamorphosed Polyp (183 hpf)", lifestage))%>%
  mutate(lifestage=if_else(lifestage=="Metamorphosed Recruit (231 hpf)", "Metamorphosed Polyp (231 hpf)", lifestage))

nrow(bacteria)
#36 symbiont DIVs 

#transpose to have samples in rows and taxa in columns
dim(bacteria)

bacteria_t<-t(bacteria)

dim(bacteria_t)

rownames(bacteria_t)
```





SOMETHING ISN'T RIGHT WITH DATA FRAME - NOT ADDING UP TO 100%, CHECK 16S DATA FRAME THAT IS INPUT - maybe not 100% since rare taxa took up a lot of space? 

Maybe don't have plsda for all and just do a circos plot right off the bat?



```{r}
#correct for lifestage names to match metabolomics
bacteria_t<-as.data.frame(bacteria_t)
dim(bacteria_t)

bacteria_t$Lifestage<-bacteria_metadata$lifestage[match(rownames(bacteria_t), bacteria_metadata$sample)]
bacteria_t$Lifestage

#summarize taxa abundance for each lifestage
bacteria_data<-bacteria_t%>%
  relocate(lifestage) #%>%
  #group_by(Lifestage)%>%
  #summarise_at(vars(2:15797), mean, na.rm = TRUE)
head(its2_data)

its2_data<-as.data.frame(its2_data)

#rownames(its2_data)<-its2_data$Lifestage

#its2_data<-its2_data%>%
  #dplyr::select(!Lifestage)

dim(its2_data)

```

Create PLSDA
```{r}
#assigning datasets 
X <- its2_data[2:64]

Y <- as.factor(its2_data$Lifestage) #select treatment names
Y

MyResult.plsda_its2 <- plsda(X,Y, ncomp=9) #number of components is classes-1

plotIndiv(MyResult.plsda_its2) 
```

Run SPLSDA to tune parameters

Plot errors
```{r}

MyPerf.plsda_its2 <- perf(MyResult.plsda_its2, validation = "Mfold", folds=3, #fold #? 
                     progressBar = TRUE, nrepeat = 10) # we suggest nrepeat = 50

plot(MyPerf.plsda_its2,  sd = TRUE)
```

Determine keepX
```{r}
#now run tune splsda 
list.keepX <- c(2:10,  seq(20, 70, 10))

set.seed(30) # for reproducible in this vignette, otherwise increase nrepeat
tune.splsda.srbct_its2 <- tune.splsda(X, Y, ncomp = 9, 
                                 validation = 'Mfold', folds=3,
                                 measure = "BER", test.keepX = list.keepX, nrepeat=20)   

error <- tune.splsda.srbct_its2$error.rate
ncomp.its2 <- tune.splsda.srbct_its2$choice.ncomp$ncomp # optimal number of components based on t-tests on the error rate
ncomp.its2

select.keepX.its2 <- tune.splsda.srbct_its2$choice.keepX# optimal number of variables to select
select.keepX.its2

plot(tune.splsda.srbct_its2)
```

The optimal number of components is ncomp=9 and keepX as the following.  

comp1 comp2 comp3 comp4 comp5 comp6 comp7 comp8 comp9 
   30     7    60    10    20    20     7    20     7 
    
# **Load data for DIABLO**  

We need to load data to have samples in rows and features in columns. 

We will calculate averages for each dataframe for each timepoint.  

Metadata 
```{r}
metadata<-read.csv("Mcap2020/Data/lifestage_metadata.csv")

genes_metadata <- read.csv("Mcap2020/Data/TagSeq/Sample_Info.csv", header = TRUE, sep = ",")
head(genes_metadata)

its2_metadata <- read.csv("Mcap2020/Data/ITS2/ITS2_Metadata.csv", header = TRUE, sep = ",")
head(its2_metadata)
```

Metabolomics
```{r}
metabolomics<-read.csv("Mcap2020/Data/Integration/selected_metabolites.csv")

metabolomics<-metabolomics%>%
  dplyr::select(Lifestage, compound, norm_counts)%>%
  group_by(Lifestage, compound)%>%
  summarise(metab=mean(norm_counts))%>%
  spread(compound, metab)

metabolomics<-as.data.frame(metabolomics)

rownames(metabolomics)<-metabolomics$Lifestage #set row names 

metabolomics<-metabolomics[-c(1)]

head(metabolomics)
```

Gene expression 
```{r}
genes<-read.csv("Mcap2020/Output/TagSeq/transformed_filtered_genes.csv") 

nrow(genes)
#15797 genes 

rownames(genes)<-genes$X

genes<-genes[-c(1)]

#transpose to have samples in rows and genes in columns
dim(genes)

genes_t<-t(genes)

dim(genes_t)

rownames(genes_t)
```


```{r}
#correct for lifestage names to match metabolomics
genes_t<-as.data.frame(genes_t)
dim(genes_t)

genes_t$Lifestage<-genes_metadata$code[match(rownames(genes_t), genes_metadata$sample_id)]
dim(genes_t)

#summarize gene counts for each lifestage
gene_data<-genes_t%>%
  relocate(Lifestage)%>%
  group_by(Lifestage)%>%
  summarise_at(vars(2:15797), mean, na.rm = TRUE)

#finally make the lifestage the row names
gene_data<-as.data.frame(gene_data)

rownames(gene_data)<-gene_data$Lifestage

gene_data<-gene_data%>%
  dplyr::select(!Lifestage)

dim(gene_data)

```

ITS2 symbiont DIVs 

```{r}
its2_metadata <- read.csv("Mcap2020/Data/ITS2/ITS2_Metadata.csv", header = TRUE, sep = ",")

its2<-read.csv("Mcap2020/Data/Integration/its2_div.csv") 

nrow(its2)
#63 symbiont DIVs 

rownames(its2)<-its2$X

its2<-its2[-c(1)]

#transpose to have samples in rows and genes in columns
dim(its2)

its2_t<-t(its2)

dim(its2_t)

rownames(its2_t)
```

```{r}
#correct for lifestage names to match metabolomics
its2_t<-as.data.frame(its2_t)
dim(its2_t)

its2_t$Lifestage<-its2_metadata$code[match(rownames(its2_t), its2_metadata$sample_name)]
dim(its2_t)

#summarize gene counts for each lifestage
its2_data<-its2_t%>%
  relocate(Lifestage) %>%
  group_by(Lifestage)%>%
  summarise_at(vars(1:63), mean, na.rm = TRUE)

head(its2_data)

its2_data<-as.data.frame(its2_data)

#rename Egg Fertilized to Egg and rename Embryo 1 to Embryo 
its2_data<-its2_data%>%
  mutate(Lifestage = if_else(Lifestage == "Egg Fertilized", "Egg", 
                               if_else(Lifestage =="Embryo 1", "Embryo", Lifestage, "NA"))) 

rownames(its2_data)<-its2_data$Lifestage

its2_data<-its2_data%>%
  dplyr::select(!Lifestage)

dim(its2_data)
rownames(its2_data)

```

# **Generate multivariate PLSDA**  

Plotting all stages together.  

Assemble dataframe  
```{r}
#filter metabolite data by the lifestages available in genes 
groups<-rownames(gene_data)

metabolomics<-metabolomics%>%
  filter(rownames(.) %in% groups)

symbiont_data<-its2_data%>%
  filter(rownames(.) %in% groups)

rownames(gene_data)
rownames(metabolomics)
rownames(symbiont_data)
#orders are correct and the same across all data sets  

X <- list(genes = gene_data, # all rows are samples, columns are compounds/genes
          metabolites = metabolomics, 
          symbiont = symbiont_data)

Y <- rownames(gene_data)
summary(Y)

```

Specify test values.  This is generated from the splsda above.  
```{r}
list.keepX <- list(genes = c(2,10,2,3,300,300,140,270), metabolites = c(2,3,10,5,140,2,3,110), symbiont = c(30,7,60,10,20,20,7,20))
```

GENES: 
comp1 comp2 comp3 comp4 comp5 comp6 comp7 comp8 comp9 
    2    10     2     3   300   300   140   270   220
    
METABOLITES: 
comp1 comp2 comp3 comp4 comp5 comp6 comp7 comp8 comp9 
    2     3    10     5   140     2     3   110     8     
    
SYMBIONTS: 
comp1 comp2 comp3 comp4 comp5 comp6 comp7 comp8 comp9 
   30     7    60    10    20    20     7    20     7 
    

    
Run PLSDA
```{r}
MyResult.diablo <- block.splsda(X, Y, keepX=list.keepX, ncomp=8)
plotIndiv(MyResult.diablo) ## sample plot
#plotVar(MyResult.diablo) ## variable plot

```

Plot circos plot  

```{r}
#circos plot
#circosPlot(MyResult.diablo, cutoff=0.95, line = TRUE, showIntraLinks=FALSE, color.cor=c("red", "white"))
circosPlot(MyResult.diablo, cutoff=0.95, line = TRUE, showIntraLinks=FALSE, color.cor=c("red", "blue"))

cor_mat<-circosPlot(MyResult.diablo, cutoff=0.95, showIntraLinks=FALSE)

```

Output plot and correlation matrix.  
```{r}
#col=c(LIST COLORS HERE)

#Add color.Y = col

pdf("Mcap2020/Figures/Integration/circos0.95.pdf", width=12, height=12)
circosPlot(MyResult.diablo, cutoff=0.95, line = TRUE, showIntraLinks=FALSE, color.cor=c("red", "blue"), ncol.legend=2)
dev.off()
```

Export correlation matrix.  
```{r}
test<-cor_mat
full<-cor_mat

test[test <0.95]<-NA
  
write.csv(test, "Mcap2020/Output/Integration/cor_circos_0.95.csv")
write.csv(full, "Mcap2020/Output/Integration/cor_circos_full.csv")
```

Ground-truthing some of our correlations: 

For example, the metabolite FAD is positively correlated with 
- g56275 (hydrolase activity) * this is expected since it is a hydrolase enzyme
- g28147	
- g68777
- g49267 (rRNA (guanine) methyltransferase activity; rRNA (guanine-N7)-methylation;NA) *this makes sense! FAD dependent methyltransferase
- g26427 (protein binding;NA) *this makes sense, its an enzyme 

Methionine sulfoxide is positively correlated with
- g11665
- g41679 (extracellular matrix structural constituent;NA) * makes sense, involved in structure/membranes 
- g46335 (extracellular matrix structural constituent;NA)
- g52872 (serine-type endopeptidase inhibitor activity; insulin-like growth factor binding; extracellular region;NA) 

NAD+ is correlated with 
- g49267 (rRNA (guanine) methyltransferase activity; rRNA (guanine-N7)-methylation;NA) *plays a role in methylation

Acetyl CoA correlated with: 
- g38364 (DNA binding, regulation of transcription) *acetyl coa can stimulate transcription, makes sense 


Plot heatmap  
```{r}
library(pheatmap)

pheatmap::pheatmap(cor_mat, cluster_rows = TRUE, cluster_cols = TRUE, show_rownames = FALSE, show_colnames = FALSE)
```

Plot cim plot  
```{r}
pdf("Mcap2020/Figures/Integration/cim_plot.pdf", height=8, width=24)
cimDiablo(MyResult.diablo, margin=c(8,8), legend.position = "none", comp = c(1,2,3,4,5,6,7,8), trim=2)
dev.off()

res <- cimDiablo(MyResult.diablo) ## save the output
res_mat<-res$mat
#can use this to pull out correlations with groups (this is super great!)

write.csv(res_mat, "Mcap2020/Output/Integration/cim_matrix.csv")
```

Trim is euclidian distance limit.  









# **Generate multivariate PLSDA with only genes and metabolites**  

Plotting all stages together.  

Assemble dataframe  
```{r}
#filter metabolite data by the lifestages available in genes 
groups<-rownames(gene_data)

metabolomics<-metabolomics%>%
  filter(rownames(.) %in% groups)

rownames(gene_data)
rownames(metabolomics)
#orders are correct and the same across all data sets  

X <- list(genes = gene_data, # all rows are samples, columns are compounds/genes
          metabolites = metabolomics)

Y <- rownames(gene_data)
summary(Y)

```

Specify test values.  This is generated from the splsda above.  
```{r}
list.keepX <- list(genes = c(2,10,2,3,300,300,140,270), metabolites = c(2,3,10,5,140,2,3,110))
```

GENES: 
comp1 comp2 comp3 comp4 comp5 comp6 comp7 comp8 comp9 
    2    10     2     3   300   300   140   270   220
    
METABOLITES: 
comp1 comp2 comp3 comp4 comp5 comp6 comp7 comp8 comp9 
    2     3    10     5   140     2     3   110     8     
    
    
Run PLSDA
```{r}
MyResult.diablo <- block.splsda(X, Y, keepX=list.keepX, ncomp=8)
plotIndiv(MyResult.diablo) ## sample plot
#plotVar(MyResult.diablo) ## variable plot

```

# Plot circos plot  

```{r}
#circos plot
#circosPlot(MyResult.diablo, cutoff=0.95, line = TRUE, showIntraLinks=FALSE, color.cor=c("red", "white"))
circosPlot(MyResult.diablo, cutoff=0.95, line = TRUE, showIntraLinks=FALSE, color.cor=c("red", "blue"))

cor_mat<-circosPlot(MyResult.diablo, cutoff=0.95, showIntraLinks=FALSE)

```

Output plot and correlation matrix.  
```{r}
#col=c(LIST COLORS HERE)

#Add color.Y = col

pdf("Mcap2020/Figures/Integration/circos0.95.pdf", width=12, height=12)
circosPlot(MyResult.diablo, cutoff=0.95, line = TRUE, showIntraLinks=FALSE, color.cor=c("red", "blue"), ncol.legend=2)
dev.off()
```

Export correlation matrix.  
```{r}
test<-cor_mat
full<-cor_mat

test[test <0.95]<-NA
  
write.csv(test, "Mcap2020/Output/Integration/cor_circos_0.95.csv")
write.csv(full, "Mcap2020/Output/Integration/cor_circos_full.csv")
```

Plot heatmap  
```{r}
library(pheatmap)

pheatmap::pheatmap(cor_mat, cluster_rows = TRUE, cluster_cols = TRUE, show_rownames = FALSE, show_colnames = FALSE)
```

Plot cim plot  
```{r}
pdf("Mcap2020/Figures/Integration/cim_plot.pdf", height=8, width=24)
cimDiablo(MyResult.diablo, margin=c(8,8), legend.position = "none", comp = c(1,2,3,4,5,6,7,8), trim=2)
dev.off()

res <- cimDiablo(MyResult.diablo) ## save the output
res_mat<-res$mat
#can use this to pull out correlations with groups (this is super great!)

write.csv(res_mat, "Mcap2020/Output/Integration/cim_matrix.csv")
```

Trim is euclidian distance limit.  

# **Generate Sankey plot**  

Need to generate correlation matrix between just genes and metabolites (that doesnt include intra correlations)??  

```{r}
head(full)

library(networkD3)

data<-full 

# Transform it to connection data frame with tidyr from the tidyverse:
links <- data %>% 
  as.data.frame() %>% 
  rownames_to_column(var="source") %>% 
  gather(key="target", value="value", -1) %>%
  filter(value != 0)
 
# From these flows we need to create a node data frame: it lists every entities involved in the flow
nodes <- data.frame(
  name=c(as.character(links$source), as.character(links$target)) %>% 
    unique()
  )
 
# With networkD3, connection must be provided using id, not using real name like in the links dataframe.. So we need to reformat it.
links$IDsource <- match(links$source, nodes$name)-1 
links$IDtarget <- match(links$target, nodes$name)-1
 
# Make the Network
p <- sankeyNetwork(Links = links, Nodes = nodes,
                     Source = "IDsource", Target = "IDtarget",
                     Value = "value", NodeID = "name", 
                     sinksRight=FALSE)

p
```

