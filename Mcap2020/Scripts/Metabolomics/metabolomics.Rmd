...
grep("(SIM)", levels(peaks_selected$compound)) #4 metabolites with HAc+, need to rename

peaks_selected<- droplevels(peaks_selected)

length(levels(peaks_selected$compound)) #199 metabolites in data set before cleaning

peaks_selected_clean<-peaks_selected[!grepl("PosIS", peaks_selected$compound),]
peaks_selected_clean<-peaks_selected_clean[!grepl("NegIS", peaks_selected_clean$compound),]
peaks_selected_clean<-droplevels(peaks_selected_clean)

length(levels(peaks_selected_clean$compound)) #185 metabolites in data set after cleaning (199-5-9=185), correct value

peaks_selected_clean<-peaks_selected_clean%>%
  mutate(compound = str_remove(compound, pattern=fixed('(+HAc)')))%>%
...
X<-X[5:189] #pull only data columns
...
X<-X[4:188] #pull only data columns
...
EvL_pvalues<-EvL_pvalues%>%
  select(compound, direction, lifestage)%>%
...
```{r}
list<-c("Larvae", "Metamorphosed Recruit")
#assigning datasets 
X <- metabolites%>%
  filter(Development %in% list)%>%
  droplevels()%>%
...
LvM_t.test.sig.up$lifestage<-c("Metamorphosed Recruit")
```
...
```{r}
list<-c("Calcifying Recruit", "Larvae")
...
LvC_t.test.sig.down$lifestage<-c("CalcifyingRecruit")
```
...
LvC_bar<-LvC_VIP_plot %>%
  ...
  scale_y_continuous(limits=c(-2, 2), breaks=c(-1,0,1))+
  xlab("") +
  annotate("text", x = 10, y = 1.6, label = "Calcifying Recruit > \nLarvae", fontface =2, color="#BA55D3") +
  annotate("text", x = 60, y = -1.6, label = "Larvae > \nCalcifying Recruit", fontface =2, color="#80CDC1") +
  #ggtitle(expression(bold("Eggs < Embryos"))) +
  theme_bw() + theme(panel.border = element_rect(linetype = "solid", color = "black"), panel.grid.major = element_blank(), #Makes background theme white
...
ggsave("Mcap2020/Figures/Metabolomics/LarvaeCalcifying_VIP_bar.pdf", width=12, height=4, units="in")
...
```{r}
list<-c("Calcifying Recruit", "Metamorphosed Recruit")
#assigning datasets 
X <- metabolites%>%
  filter(Development %in% list)%>%
  droplevels()%>%
...
MvC_t.test.sig.up$lifestage<-c("MetamorphosedRecruit")
```
...
MvC_t.test.sig.down$lifestage<-c("CalcifyingRecruit")
```
...
MvC_bar<-MvC_VIP_plot %>%
  ...
  scale_y_continuous(limits=c(-2, 2), breaks=c(-1,0,1))+
  xlab("") +
  annotate("text", x = 10, y = 1.6, label = "Calcifying Recruit > \nMetamorphosed Recruit", fontface =2, color="#BA55D3") +
  annotate("text", x = 50, y = -1.6, label = "Metamorphosed Recruit > \nCalcifying Recruit", fontface =2, color="#003C30") +
  #ggtitle(expression(bold("Eggs < Embryos"))) +
  theme_bw() + theme(panel.border = element_rect(linetype = "solid", color = "black"), panel.grid.major = element_blank(), #Makes background theme white
...
### 7. Generate combined file for pathway analysis  

Generate a combined file of all enriched metabolites (based on VIP calculations) for all lifestages and remove non significant metabolites. Export file for each lifestage for separate analysis.    

```{r}
kegg_file<-rbind(EvE_VIP_plot, EvL_VIP_plot, LvC_VIP_plot, LvM_VIP_plot, MvC_VIP_plot)

kegg_file<-kegg_file%>%
  filter(!direction=="Nonsig")%>%
  select(Metabolite, lifestage)

kegg_egg<-kegg_file%>%
  filter(lifestage=="Egg")%>%
  write.csv(., "Mcap2020/Output/Metabolomics/KEGG_egg.csv")

kegg_embryo<-kegg_file%>%
  filter(lifestage=="Embryo")%>%
  write.csv(., "Mcap2020/Output/Metabolomics/KEGG_embryo.csv")

kegg_larvae<-kegg_file%>%
  filter(lifestage=="Larvae")%>%
  write.csv(., "Mcap2020/Output/Metabolomics/KEGG_larvae.csv")

kegg_metamorphosed<-kegg_file%>%
  filter(lifestage=="MetamorphosedRecruit")%>%
  write.csv(., "Mcap2020/Output/Metabolomics/KEGG_metamorphosed.csv")

kegg_calcifying<-kegg_file%>%
  filter(lifestage=="CalcifyingRecruit")%>%
  write.csv(., "Mcap2020/Output/Metabolomics/KEGG_calcifying.csv")
```
...
For development, we get the most information from 4 of the 5 components (larvae and metamorphosed recruits overlap).   
...
ind.match4 = match(selectVar(myResult.splsda.final_D, comp = 4)$name, 
                  names(perf.splsda$features$stable[[4]]))
ind.match5 = match(selectVar(myResult.splsda.final_D, comp = 5)$name, 
                  names(perf.splsda$features$stable[[5]]))
...
Freq3 = as.numeric(perf.splsda$features$stable[[3]][ind.match3])
Freq4 = as.numeric(perf.splsda$features$stable[[4]][ind.match4])
Freq5 = as.numeric(perf.splsda$features$stable[[5]][ind.match4])
...
comp5.select.metabolites <- data.frame(selectVar(myResult.splsda.final_D, comp = 5)$value, Freq5)
