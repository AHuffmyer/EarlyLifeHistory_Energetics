...
dim(DEG_results)
```
There are 9181 DEGs out of 11,475 total genes (80%). Most genes are differentially expressed across time. This is expected for a developmental time series spanning eggs to recruits due to the drastic changes in function.  
...
```{r}
dev.off()
pdf("Mcap2020/Figures/TagSeq/GenomeV3/DESeq/DEG_heatmap_development.pdf", width=12, height=14)
...
scaled_test_DEG <-prcomp(test_DEG[c(1:9181)], scale=TRUE, center=TRUE)
fviz_eig(scaled_test_DEG)

# scale data
vegan_DEG <- scale(test_DEG[c(1:9181)])
...
adonis2(formula = vegan_DEG ~ lifestage, data = test_DEG, method = "eu")
          Df SumOfSqs      R2     F Pr(>F)    
lifestage  9   272041 0.80083 12.51  0.001 ***
Residual  28    67656 0.19917                 
Total     37   339697 1.00000  

Plot PCA.  
```{r}
gPCAdata_DEG <- plotPCA(DEG_results_vst, intgroup = c("lifestage"), returnData=TRUE, ntop=9181) #use ntop to specify all genes
...
Filter for genes padj <0.05. For LRT, the p-value is the cut off associated with the test statistic. Fold change is not associated with the statistical test. It can be viewed, but you cannot select genes based on a fold change cut off to determine DEGs.  
...
nrow(sigLRT_genes)
```
There are 9181 genes that change across developmental time at p<0.05.    
...
```
6982 genes were clustered with cluster size >100 for all DEGS (9181 total)
8283 genes clusters with cluster size >50 
8900 genes clusters with cluster size >25
9072 genes clusters with cluster size >15 (98.8% of all DEGs were clustered)
...
ggsave(cluster_plot, filename="Mcap2020/Figures/TagSeq/GenomeV3/DESeq/cluster_plot.pdf", width=12, height=10)
```
...
Identified 64 clusters of genes sharing expression patterns. 

These are categorized in developmental stages as clusters that show a peak in the respective developmental periods. 

If a cluster peaks in two developmental phases, include in both. Include the cluster if peaking in one of these three developmental stages. 

```{r}
#early development 
early<-c("4", "7", "10", "11", "15", "16", "18", "21", "22", "24", "25", "29", "32", "35", "36", "43", "45", "52", "54", "57", "59", "65", "66")

#mid development 
mid<-c("3", "5", "8", "9", "13", "14", "17", "19", "23", "27", "28", "31", "38", "40", "46", "48", "49", "51", "55", "60", "63", "64", "67")

#late development
late<-c("1", "2", "5", "6", "12", "17", "19", "20", "26", "30", "33", "34", "42", "44", "47", "50", "53", "56", "58", "62", "70")
...
ggsave(cluster_plot2, filename="Mcap2020/Figures/TagSeq/GenomeV3/DESeq/cluster_plot_patterns.pdf", width=7, height=3.5)
```
...
#2679 genes in early development 
#3192 genes in late development 
#3201 genes in mid development
```
9072 clustered total 
...
group_list%>%
      ...
      write_csv(file="Mcap2020/Output/TagSeq/GenomeV3/DESeq/cluster_counts.csv")
...
```
There are 485 total enriched GO terms detected when analyzed by pattern groups. 
...
ggsave(filename="Mcap2020/Figures/TagSeq/GenomeV3/DESeq/functional_enrichment/go_early.pdf", go_plot_early, width=35, height=48)
...
ggsave(filename="Mcap2020/Figures/TagSeq/GenomeV3/DESeq/functional_enrichment/go_mid.pdf", go_plot_mid, width=12, height=15)
...
ggsave(filename="Mcap2020/Figures/TagSeq/GenomeV3/DESeq/functional_enrichment/go_late.pdf", go_plot_late, width=22, height=12)
