...
Why am I getting so many STRG genes from my stringtie step????
...
Generate a file with all genes and

```{r}
all<-geneInfo%>%
  dplyr::select(gene_id, Length, Annotation.GO.ID)
```
...
module_list<-geneInfo%>%
  dplyr::select(gene_id, Length, Annotation.GO.ID, Module)

levels(module_list$Module)

module1<-module_list%>%filter(Module=="1")
module2<-module_list%>%filter(Module=="2")
module3<-module_list%>%filter(Module=="3")
module5<-module_list%>%filter(Module=="5")
module6<-module_list%>%filter(Module=="6")
module7<-module_list%>%filter(Module=="7")
module9<-module_list%>%filter(Module=="9")
module10<-module_list%>%filter(Module=="10")
...
module19<-module_list%>%filter(Module=="19")
```
...
GO.terms <- module1%>%
  select(gene_id, Annotation.GO.ID)
...
GO_all.BP <-  ggplot(go_all, aes(x = Lifestage, y = term)) + 
  ...
  scale_size(name="Over rep. p-value", range = c(10, 20), trans="reverse")+
...
```{r}
go_all.MF<-rbind(eggs.GO_plot.MF, embryos.GO_plot.MF, larvae1.GO_plot.MF,  larvae3.GO_plot.MF, larvae4.GO_plot.MF, larvae6.GO_plot.MF, polyps.GO_plot.MF, recruit1.GO_plot.MF, recruit2.GO_plot.MF)
...
high_late_list<-c("12", "15", "8")
high_mid_list<-c("16", "2", "4", "18")
#ME14 and ME 17 don't fit a pattern 
...
ggsave(filename="Mcap2020/Figures/TagSeq/GOSeq/Patterns/GOenrich_pattern_high_early_BP.png", plot=GO_high_early.plot, dpi=100, width=12, height=12, units="in")
```
...
ggsave(filename="Mcap2020/Figures/TagSeq/GOSeq/Patterns/GOenrich_pattern_high_early_MF.png", plot=GO_high_early.plot.MF, dpi=100, width=12, height=12, units="in")
```
...
ggsave(filename="Mcap2020/Figures/TagSeq/GOSeq/Patterns/GOenrich_pattern_high_late_BP.png", plot=GO_high_late.plot, dpi=100, width=12, height=12, units="in")
```
...
ggsave(filename="Mcap2020/Figures/TagSeq/GOSeq/Patterns/GOenrich_pattern_high_late_MF.png", plot=GO_high_late.plot.MF, dpi=100, width=12, height=12, units="in")
```
...
ggsave(filename="Mcap2020/Figures/TagSeq/GOSeq/Patterns/GOenrich_pattern_high_mid_BP.png", plot=GO_high_mid.plot, dpi=100, width=12, height=12, units="in")
```
...
ggsave(filename="Mcap2020/Figures/TagSeq/GOSeq/Patterns/GOenrich_pattern_high_mid_MF.png", plot=GO_high_mid.plot.MF, dpi=100, width=12, height=12, units="in")
```
...
GO_all.BP <-  ggplot(go_all, aes(x = Pattern, y = term)) + 
  geom_point(aes(colour=Pattern, size=over_represented_pvalue)) +
  facet_grid(GOSlim_bin ~ ., scales = "free", space= "free", shrink=TRUE, labeller = label_wrap_gen(width=30, multi_line = TRUE))+
  scale_colour_manual(name="Ontogenetic Pattern", values = c("High Early in Development"="lightgray", "High in Mid Development"="darkgray", "High Late in Development"="black")) +
  scale_size(name="Over rep. p-value", range = c(10, 18), trans="reverse")+
  guides(colour = guide_legend(override.aes = list(size=14)))+
...
ggsave(filename="Mcap2020/Figures/TagSeq/GOSeq/Patterns/GOenrich_all_patterns_BP.png", plot=GO_all.BP, dpi=50, width=30, height=40, units="in")
```
...
GO_all.MF <-  ggplot(go_all.MF, aes(x = Pattern, y = term)) + 
  ...
  facet_grid(GOSlim_bin ~ ., scales = "free", space= "free", shrink=TRUE, labeller = label_wrap_gen(width=30, multi_line = TRUE))+
  scale_colour_manual(name="Ontogenetic Pattern", values = c("High Early in Development"="lightgray", "High in Mid Development"="darkgray", "High Late in Development"="black")) +
  scale_size(name="Over rep. p-value", range = c(8, 14), trans="reverse")+
  guides(colour = guide_legend(override.aes = list(size=14)))+
  xlab("Ontogenetic Pattern")+
  ylab("Molecular Function GO Term")+
  theme_bw() + theme(panel.border = element_blank(), panel.grid.major = element_blank(),
...
ggsave(filename="Mcap2020/Figures/TagSeq/GOSeq/Patterns/GOenrich_all_patterns_MF.png", plot=GO_all.MF, dpi=50 , width=40, height=49, units="in") 
```

## Plot trends of eigenegenes for each ontogenetic pattern  

Plot eigengene values for each pattern identified above from the module eigengene expression values.  

```{r}
pattern_expression<-read_csv("Mcap2020/Output/TagSeq/WGCNA_eigengene_expression.csv")

#add pattern identifier to each module 
high_early_list_m<-c("ME1", "ME11", "ME6")
high_late_list_m<-c("ME12", "ME15", "ME8")
high_mid_list_m<-c("ME16", "ME2", "ME4", "ME18")

pattern_expression<-pattern_expression%>%
  mutate(Pattern=if_else(Module %in% high_early_list_m, "High Early in Development", "NA"))%>%
  mutate(Pattern=if_else(Module %in% high_late_list_m, "High Late in Development", Pattern))%>%
  mutate(Pattern=if_else(Module %in% high_mid_list_m, "High in Mid Development", Pattern))
...
#Gene counts from WGCNA_Mcap.Rmd script
gene_counts<-read_csv("Mcap2020/Output/TagSeq/genes_per_module.csv")
names(gene_counts)<-c("Module", "Genes")

#add pattern group for each module  
gene_counts<-gene_counts%>%
  mutate(Pattern=if_else(Module %in% high_early_list, "High Early in Development", "NA"))%>%
  mutate(Pattern=if_else(Module %in% high_late_list, "High Late in Development", Pattern))%>%
  mutate(Pattern=if_else(Module %in% high_mid_list, "High in Mid Development", Pattern))
...
There are 3 modules in early development, 4 in mid development, and 3 in late development.  

There are 5159 genes in early, 4682 in mid, and 5844 in late development patterns.  

```{r}
f_labels <- data.frame(Pattern = c("High Early in Development", "High in Mid Development", "High Late in Development"), label = c("n=3 modules; n=5159 genes", "n=4 modules; n=4682 genes", "n=3 modules; n=5844 genes"))
f_labels
```
...
pattern_plots<-pattern_expression%>%
  ...
  facet_wrap(~Pattern, nrow=3, ncol=1)+
  ...
  theme(axis.text.x=element_text(angle = 45, hjust=1, size = 12), #set x-axis label size
        ...
        plot.title = element_text(size=22)); pattern_plots

ggsave(filename="Mcap2020/Figures/TagSeq/GOSeq/Patterns/ontogeny_patterns_expression.pdf", plot=pattern_plots, dpi=50 , width=6, height=10, units="in")
```
