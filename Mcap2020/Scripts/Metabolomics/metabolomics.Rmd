...
data_pca<-data_pca%>%
  filter(!(Sample=="Larvae2_rep1_norm"))%>%
  filter(!(Sample=="Larvae2_rep2_norm"))%>%
  filter(!(Sample=="Larvae4_rep1_norm"))%>%
  filter(!(Sample=="Larvae4_rep2_norm"))%>%
  filter(!(Sample=="Larvae6_rep1_norm"))%>%
  filter(!(Sample=="Larvae6_rep2_norm"))
...
```{r}
scaled.pca<-prcomp(data_pca[c(4:131)],scale=TRUE, center=TRUE) 
...
```{r}
scaled.pca.neg<-prcomp(data_pca_neg[c(4:139)],scale=TRUE, center=TRUE) 
...
data_pca_full<-data_pca_full%>%
  ...
  filter(!(Sample=="Larvae2 rep1"))%>%
  filter(!(Sample=="Larvae2 rep2"))%>%
  filter(!(Sample=="Larvae4 rep1"))%>%
  filter(!(Sample=="Larvae4 rep2"))%>%
  filter(!(Sample=="Larvae6 rep1"))%>%
  filter(!(Sample=="Larvae6 rep2"))
...
```{r}
scaled.pca<-prcomp(data_pca_full[c(3:97)],scale=TRUE, center=TRUE) 
...
Y <- as.factor(X$Lifestage) #select treatment names

X<-X[3:199] #pull only data columns
