...
file2<-read_csv("Mcap2020/Data/Integration/RelativeChange/cells_relchange.csv")%>%
  dplyr::select(!group)%>%
   dplyr::rename("Symbiont Density"="sym_density_ind", "Symbiont Cells per mm2"="sym_density_mm")%>%
  gather(2:3, key="variable", value="value")
  
#NEED TO ADD FUNCTIONAL INFORMATION TO THESE CATEGORIES 
#genes increasing: carb and lipid metabolism, ROS regulation and organic transport
#genes mid: carb and lipid metabolism, ROS regulation and organic transport
#genes decreasing: amino acid and steroid metabolism

file3<-read_csv("Mcap2020/Data/Integration/RelativeChange/genes_relchange.csv")%>%
  mutate(Pattern=if_else(Pattern=="Decreasing", "Amino acid and steroid metabolism", 
                         if_else(Pattern=="Increasing", "Carbohydrate and lipid metabolism, ROS regulation, organic transport", 
                                 if_else(Pattern=="Mid", "Carbohydrate and lipid metabolism, ROS regulation, transport", "NA"))))%>%
   dplyr::rename("variable"="Pattern", "value"="gene_expression")%>%
  filter(value<10)

#NEED TO ADD FUNCTIONAL INFORMATION TO THESE CATEGORIES 
#metabolites increasing: carbohydrate metabolism, amino acid synthesis
#metabolites mid: nitrogen metabolism
#metabolites decreasing: amino acid metabolism

file4<-read_csv("Mcap2020/Data/Integration/RelativeChange/metabolites_relchange.csv")%>%
  mutate(variable=if_else(pattern=="Decreasing", "Amino acid metabolism", 
                         if_else(pattern=="Increasing", "Carbohydrate metabolism, amino acid synthesis", 
                                 if_else(pattern=="Mid", "Nitrogen metabolism", "NA"))))%>%
   dplyr::rename("value"="metabolite_expression")%>%
  dplyr::select(!pattern)

file5<-read_csv("Mcap2020/Data/Integration/RelativeChange/ITS2_CD_relchange.csv")%>%
  gather(key="variable", value="value", symbiont_CD_ratio)%>%
  mutate(variable="Symbiont C:D Ratio")

file6<-read_csv("Mcap2020/Data/Integration/RelativeChange/PR_relchange.csv")%>%
   dplyr::rename("lifestage"="lifestage_std", "Gross Photosynthesis"="GP_change", "Respiration"="R_change", "P:R"="ratio_change")%>%
...
file7<-read_csv("Mcap2020/Data/Integration/RelativeChange/size_relchange.csv")%>%
  ...
  dplyr::rename("lifestage"="Lifestage")%>%
...
df<-df%>%
  mutate(category=if_else(variable=="Bacterial Alpha Diversity", "Microbiome Diversity", 
                          if_else(variable=="Symbiont C:D Ratio", "Symbiont Community", 
                                  if_else(variable=="Amino acid and steroid metabolism", "Molecular Response", 
                                          if_else(variable=="Carbohydrate and lipid metabolism, ROS regulation, organic transport", "Molecular Response",
                                                  if_else(variable=="Carbohydrate and lipid metabolism, ROS regulation, transport", "Molecular Response",
                                                          if_else(variable=="Amino acid metabolism", "Metabolomic Response",
                                                                  if_else(variable=="Carbohydrate metabolism, amino acid synthesis", "Metabolomic Response",
                                                                          if_else(variable=="Nitrogen metabolism", "Metabolomic Response",
                                                                                  if_else(variable=="Gross Photosynthesis", "Photosynthetic Capacity",
                                                                                          if_else(variable=="Symbiont Density", "Photosynthetic Capacity",
...
#microbiome: bacteria diversity(#f0f921)
#molecular response (#cc4778)
#metabolomic response (#0d0887)
#symbiont: XXX

color_keys<-c("Energy Demand"="black", "Photosynthetic Capacity"="orange", "Microbiome Diversity"="darkgray", "Molecular Response"="red", "Metabolomic Response"="blue", "Symbiont Community"="darkgreen")
...
ggsave("Mcap2020/Figures/Integration/RelativeChange.pdf", change_plot, width=12, height=7, dpi=300)
```
