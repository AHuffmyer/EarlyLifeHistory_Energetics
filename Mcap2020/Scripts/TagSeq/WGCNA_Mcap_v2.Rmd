...
This script conducts WGCNA and DEG analysis of TagSeq data set using version 2 of the *M. capitata* genome made available through Rutgers (http://cyanophora.rutgers.edu/montipora/).  
...
nrow(gcount)
```
There were about 20,000 genes that had 0 across all rows. These were removed. We now have 34,667 genes.    
...
Before filtering, we had approximately 35,000 genes. After filtering for pOverA, we have approximately 10,600 genes. This indicates that there were many low count genes present in <7% of samples at <10 counts per gene.  

In order for the DESeq2 algorithms to work, the SampleIDs on the metadta file and count matrices have to match exactly and in the same order. 
...
save_pheatmap_pdf(pht, "Mcap2020/Figures/TagSeq/GenomeV2/pheatmap.pdf")
```
...
pdf("Mcap2020/Figures/TagSeq/GenomeV2/outliers.pdf")
...
pdf("Mcap2020/Figures/TagSeq/GenomeV2/blockwise_module_colors.pdf")
plotDendroAndColors(
  netwk$dendrograms[[1]],
...
deepSplit=3 gives us 11 modules with 5685 in module 0
deepSplit=2 gives us 7 modules with 3319 in module 0

deepSplit=2 with mergecut=0.15 gives us 7 modules
deepSplit=2 with mergecut=0.1 gives us 7 modules

deepSplit=3 with mergecut=0.15 gives us 10 modules
deepSplit=3 with mergecut=0.1 gives us 11 modules 





WHY AM I GETTING DIFFERENT MODULE RESULTS THAN WITH THE V1 OF THE GENOME???? 

BLOCKWISE MODULE RESULTS FROM V1 OF THE GENOME: 
