...
Betaine could be a large outlier, likely due to very high abundance compared to other metabolites. Try analysis with and without Betaine. Proceeding now without removing Betaine because there are no sample outliers including betaine in analysis below.      
...
```{r}
list_groups<-c("Eggs & Embryos", "Larvae & Metamorphosed Polyps", "Attached Recruits")
sig_pathways$Group<-factor(as.character(sig_pathways$Group), levels=list_groups)
  
#plot significantly enriched GO terms by Slim Category across lifestages
Pathways.Dot.Plot <-  ggplot(sig_pathways, aes(x = Group, y = Metabolite.Set)) + 
  geom_point(aes(colour=Group, size=FDR)) +
  scale_colour_manual(name="Lifestage Cluster", values = c("Eggs & Embryos"="lightgray", "Larvae & Metamorphosed Polyps"="darkgray", "Attached Recruits"="black")) +
  scale_size(name="FDR p-value", range = c(10, 25), trans="reverse")+
  guides(colour = guide_legend(override.aes = list(size=18)))+
  xlab("Lifestage Cluster")+
  ylab("KEGG Pathway")+
...
ggsave(filename="Mcap2020/Figures/Metabolomics/WGCNA/pathways_dot_plot.png", plot=Pathways.Dot.Plot, dpi=80, width=25, height=20, units="in")
...
```{r}
list_groups<-c("Eggs & Embryos", "Larvae & Metamorphosed Polyps", "Attached Recruits")
sig_classes_comp$Group<-factor(as.character(sig_classes_comp$Group), levels=list_groups)
  
#plot significantly enriched GO terms by Slim Category across lifestages
Compounds.Dot.Plot <-  ggplot(sig_classes_comp, aes(x = Group, y = Metabolite.Set)) + 
  geom_point(aes(colour=Group, size=FDR)) +
...
ggsave(filename="Mcap2020/Figures/Metabolomics/WGCNA/compounds_dot_plot.png", plot=Compounds.Dot.Plot, dpi=80, width=25, height=20, units="in")

```

# 6. Plotting specific metabolites  

Format our data frame for plotting.  
```{r}
metab_expr<-as.matrix(data_filt)
metab_expr<-t(metab_expr)
metab_expr<-as.data.frame(metab_expr, row.names = rownames(metab_expr), make.names=TRUE)
```

Create list of expression of metabolites for glucose metabolism: 

```{r}
glucose_metab<-c("Glucose", "Pyruvate", "Acetyl CoA", "Glucose-6-phosphate", "Phosphoenolpyruvate")

glucose_metab<-metab_expr[rownames(metab_expr) %in% glucose_metab, ]
glucose_metab

glucose_metab$Function<-c("Glucose Metabolism")
glucose_metab$metabolite_id<-rownames(glucose_metab)
rownames(glucose_metab) <- NULL
```

Create list of expression of metabolites for fructose metabolism: 

```{r}
fructose_metab<-c("Sorbitol", "Fructose", "Mannose-6-phosphate", "GDP-Mannose","Dihydroxyacetone phosphate")

fructose_metab<-metab_expr[rownames(metab_expr) %in% fructose_metab, ]
fructose_metab

fructose_metab$Function<-c("Fructose Metabolism")
fructose_metab$metabolite_id<-rownames(fructose_metab)
rownames(fructose_metab) <- NULL
```

Create list of expression of metabolites for pyruvate metabolism: 

```{r}
pyruvate_metab<-c("Phosphoenolpyruvate", "Pyruvate","Acetyl CoA","Fumarate")

pyruvate_metab<-metab_expr[rownames(metab_expr) %in% pyruvate_metab, ]
pyruvate_metab

pyruvate_metab$Function<-c("Pyruvate Metabolism")
pyruvate_metab$metabolite_id<-rownames(pyruvate_metab)
rownames(pyruvate_metab) <- NULL
```

Create list of expression of metabolites for starch/sucrose metabolism: 

```{r}
starch_metab<-c("Cellobiose", "Fructose", "Sucrose", "UDP-D-Glucose","Glucose-6-phosphate", "Glucose")

starch_metab<-metab_expr[rownames(metab_expr) %in% starch_metab, ]
starch_metab

starch_metab$Function<-c("Starch Sucrose Metabolism")
starch_metab$metabolite_id<-rownames(starch_metab)
rownames(starch_metab) <- NULL
```

Create list of expression of metabolites for galactose metabolism: 

```{r}
galactose_metab<-c("Sucrose", "Raffinose", "UDP-D-Glucose", "Glucose-6-phosphate", "Glucose", "Fructose", "Sorbitol", "Inositol", "Galactitol")

galactose_metab<-metab_expr[rownames(metab_expr) %in% galactose_metab, ]
galactose_metab

galactose_metab$Function<-c("Galactose Metabolism")
galactose_metab$metabolite_id<-rownames(galactose_metab)
rownames(galactose_metab) <- NULL
```

Create list of expression of metabolites for citrate cycle metabolism: 

```{r}
citrate_metab<-c("a-ketoglutarate", "Succinate", "Isocitrate", "Acetyl CoA", "Citrate", "Pyruvate", "Fumarate", "Phosphoenolpyruvate")

citrate_metab<-metab_expr[rownames(metab_expr) %in% citrate_metab, ]
citrate_metab

citrate_metab$Function<-c("Citrate Cycle")
citrate_metab$metabolite_id<-rownames(citrate_metab)
rownames(citrate_metab) <- NULL
```

Create list of expression of metabolites for nitrogen metabolism: 

```{r}
nitrogen_metab<-c("Glutamate", "Carbamoyl phosphate", "Glutamine")

nitrogen_metab<-metab_expr[rownames(metab_expr) %in% nitrogen_metab, ]
nitrogen_metab

nitrogen_metab$Function<-c("Nitrogen Metabolism")
nitrogen_metab$metabolite_id<-rownames(nitrogen_metab)
rownames(nitrogen_metab) <- NULL
```

Create list of expression of metabolites for amino acids: 

```{r}
amino_metab<-c("Histidine", "Lysine", "Methionine", "Phenylalanine", "Tyrosine", "Valine")

amino_metab<-metab_expr[rownames(metab_expr) %in% amino_metab, ]
amino_metab

amino_metab$Function<-c("Amino Acids")
amino_metab$metabolite_id<-rownames(amino_metab)
rownames(amino_metab) <- NULL
```

Create list of expression of metabolites for dipeptides: 

```{r}
dipep_metab<-c("Alanine-Alanine", "Arginine-Alanine", "Arginine-Glutamine", "Arginine-Valine")

dipep_metab<-metab_expr[rownames(metab_expr) %in% dipep_metab, ]
dipep_metab

dipep_metab$Function<-c("Dipeptides")
dipep_metab$metabolite_id<-rownames(dipep_metab)
rownames(dipep_metab) <- NULL
```

Create list of expression of metabolites for methylation: 

```{r}
methylation<-c("Acetyl CoA", "S-adenosyl-L-methionine", "FAD", "NAD+")

methylation<-metab_expr[rownames(metab_expr) %in% methylation, ]
methylation

methylation$Function<-c("Methylation")
methylation$metabolite_id<-rownames(methylation)
rownames(methylation) <- NULL
```

Combine all lists together.  
```{r}
specific_metab<-rbind(glucose_metab, fructose_metab, pyruvate_metab, starch_metab, galactose_metab, citrate_metab, nitrogen_metab, amino_metab, dipep_metab, methylation)
```

Gather the data set and merge in metadata information.  

```{r}
specific_metab<-specific_metab%>%
  gather(key="Sample", value="normalized_ions", 1:47)

specific_metab$code<-metadata$Lifestage[match(specific_metab$Sample, metadata$Sample)]

#bring in developmental metadata 
keys<-read.csv("Mcap2020/Data/lifestage_metadata.csv")

specific_metab$lifestage<-keys$lifestage[match(specific_metab$code, keys$code)]

lifestage_order<-c("Egg (1 hpf)", "Embryo (5 hpf)", "Embryo (38 hpf)", "Embryo (65 hpf)", "Larvae (93 hpf)", "Larvae (163 hpf)", "Larvae (183 hpf)", "Larvae (231 hpf)", "Metamorphosed Polyp (183 hpf)", "Metamorphosed Polyp (231 hpf)", "Attached Recruit (183 hpf)", "Attached Recruit (231 hpf)", "Attached Recruit (255 hpf)")

specific_metab$lifestage<-factor(specific_metab$lifestage, levels = lifestage_order)

specific_metab$group<-keys$group[match(specific_metab$code, keys$code)]
head(specific_metab)

group_order<-c("Egg", "Embryo", "Larvae", "Metamorphosed Polyp", "Attached Recruit")

specific_metab$group<-factor(specific_metab$group, levels = group_order)
```
...
```{r}
glucose_plot<-specific_metab%>%
    group_by(Function, metabolite_id, lifestage)%>%
    summarise(mean=mean(normalized_ions), N=length(normalized_ions), sd=sd(normalized_ions), sem=sd/N)%>%
    filter(Function=="Glucose Metabolism")%>%
    ggplot(., aes(x = lifestage, y = mean, color=metabolite_id, group=metabolite_id)) +
    facet_wrap(~Function)+
    geom_line(aes(group=metabolite_id), position = position_dodge(0.1)) +
    geom_point(aes(group=metabolite_id), size=2, position = position_dodge(0.1)) + 
  geom_errorbar(aes(group=metabolite_id, ymin=mean-sem, ymax=mean+sem), width=0.1, linetype="solid", color="black", position=position_dodge(0.1), size=0.5) + 
    xlab("Lifestage") + 
...
Plot fructose metabolism.  
```{r}
fructose_plot<-specific_metab%>%
    group_by(Function, metabolite_id, lifestage)%>%
    summarise(mean=mean(normalized_ions), N=length(normalized_ions), sd=sd(normalized_ions), sem=sd/N)%>%
    filter(Function=="Fructose Metabolism")%>%
    ggplot(., aes(x = lifestage, y = mean, color=metabolite_id, group=metabolite_id)) +
    facet_wrap(~Function)+
    geom_line(aes(group=metabolite_id), position = position_dodge(0.1)) +
    geom_point(aes(group=metabolite_id), size=2, position = position_dodge(0.1)) + 
...
Plot pyruvate metabolism.  
```{r}
pyruvate_plot<-specific_metab%>%
    group_by(Function, metabolite_id, lifestage)%>%
    summarise(mean=mean(normalized_ions), N=length(normalized_ions), sd=sd(normalized_ions), sem=sd/N)%>%
    filter(Function=="Pyruvate Metabolism")%>%
    ggplot(., aes(x = lifestage, y = mean, color=metabolite_id, group=metabolite_id)) +
    facet_wrap(~Function)+
    geom_line(aes(group=metabolite_id), position = position_dodge(0.1)) +
    geom_point(aes(group=metabolite_id), size=2, position = position_dodge(0.1)) + 
...
Plot starch/sucrose metabolism.  
```{r}
starch_plot<-specific_metab%>%
    group_by(Function, metabolite_id, lifestage)%>%
    summarise(mean=mean(normalized_ions), N=length(normalized_ions), sd=sd(normalized_ions), sem=sd/N)%>%
    filter(Function=="Starch Sucrose Metabolism")%>%
    ggplot(., aes(x = lifestage, y = mean, color=metabolite_id, group=metabolite_id)) +
    facet_wrap(~Function)+
    geom_line(aes(group=metabolite_id), position = position_dodge(0.1)) +
    geom_point(aes(group=metabolite_id), size=2, position = position_dodge(0.1)) + 
...
Plot galactose metabolism.  
```{r}
galactose_plot<-specific_metab%>%
    group_by(Function, metabolite_id, lifestage)%>%
    summarise(mean=mean(normalized_ions), N=length(normalized_ions), sd=sd(normalized_ions), sem=sd/N)%>%
    filter(Function=="Galactose Metabolism")%>%
    ggplot(., aes(x = lifestage, y = mean, color=metabolite_id, group=metabolite_id)) +
    facet_wrap(~Function)+
    geom_line(aes(group=metabolite_id), position = position_dodge(0.1)) +
    geom_point(aes(group=metabolite_id), size=2, position = position_dodge(0.1)) + 
...
Plot citrate cycle   
```{r}
citrate_plot<-specific_metab%>%
    group_by(Function, metabolite_id, lifestage)%>%
    summarise(mean=mean(normalized_ions), N=length(normalized_ions), sd=sd(normalized_ions), sem=sd/N)%>%
    filter(Function=="Citrate Cycle")%>%
    ggplot(., aes(x = lifestage, y = mean, color=metabolite_id, group=metabolite_id)) +
    facet_wrap(~Function)+
    geom_line(aes(group=metabolite_id), position = position_dodge(0.1)) +
    geom_point(aes(group=metabolite_id), size=2, position = position_dodge(0.1)) + 
...
Plot nitrogen metabolism   
```{r}
nitrogen_plot<-specific_metab%>%
    group_by(Function, metabolite_id, lifestage)%>%
    summarise(mean=mean(normalized_ions), N=length(normalized_ions), sd=sd(normalized_ions), sem=sd/N)%>%
    filter(Function=="Nitrogen Metabolism")%>%
    ggplot(., aes(x = lifestage, y = mean, color=metabolite_id, group=metabolite_id)) +
    facet_wrap(~Function)+
    geom_line(aes(group=metabolite_id), position = position_dodge(0.1)) +
    geom_point(aes(group=metabolite_id), size=2, position = position_dodge(0.1)) + 
...
Plot amino acids.  
```{r}
amino_plot<-specific_metab%>%
    group_by(Function, metabolite_id, lifestage)%>%
    summarise(mean=mean(normalized_ions), N=length(normalized_ions), sd=sd(normalized_ions), sem=sd/N)%>%
    filter(Function=="Amino Acids")%>%
    ggplot(., aes(x = lifestage, y = mean, color=metabolite_id, group=metabolite_id)) +
    facet_wrap(~Function)+
    geom_line(aes(group=metabolite_id), position = position_dodge(0.1)) +
    geom_point(aes(group=metabolite_id), size=2, position = position_dodge(0.1)) + 
...
Plot dipeptides.  
```{r}
dipep_plot<-specific_metab%>%
    group_by(Function, metabolite_id, lifestage)%>%
    summarise(mean=mean(normalized_ions), N=length(normalized_ions), sd=sd(normalized_ions), sem=sd/N)%>%
    filter(Function=="Dipeptides")%>%
    ggplot(., aes(x = lifestage, y = mean, color=metabolite_id, group=metabolite_id)) +
    facet_wrap(~Function)+
    geom_line(aes(group=metabolite_id), position = position_dodge(0.1)) +
    geom_point(aes(group=metabolite_id), size=2, position = position_dodge(0.1)) + 
...
Plot methylation.  
```{r}
methylation_plot<-specific_metab%>%
    group_by(Function, metabolite_id, lifestage)%>%
    summarise(mean=mean(normalized_ions), N=length(normalized_ions), sd=sd(normalized_ions), sem=sd/N)%>%
    filter(Function=="Methylation")%>%
    ggplot(., aes(x = lifestage, y = mean, color=metabolite_id, group=metabolite_id)) +
    facet_wrap(~Function)+
    geom_line(aes(group=metabolite_id), position = position_dodge(0.1)) +
    geom_point(aes(group=metabolite_id), size=2, position = position_dodge(0.1)) + 
...
Join metabolism plots together.  

```{r}
library(cowplot)

glucose_plot2<-glucose_plot+theme(axis.text.x=element_blank(), axis.title.x=element_blank())
fructose_plot2<-fructose_plot+theme(axis.text.x=element_blank(), axis.title.x=element_blank())
pyruvate_plot2<-pyruvate_plot+theme(axis.text.x=element_blank(), axis.title.x=element_blank())
starch_plot2<-starch_plot+theme(axis.text.x=element_blank(), axis.title.x=element_blank())

metab_plots<-plot_grid(glucose_plot2, fructose_plot2, pyruvate_plot2, starch_plot2, galactose_plot, citrate_plot, nitrogen_plot, amino_plot, ncol=4, nrow=2, rel_heights = c(0.7,1), align='v')
                      

ggsave(filename="Mcap2020/Figures/Metabolomics/metabolite_expression_class.pdf", plot=metab_plots, dpi=300, width=30, height=10, units="in")
```