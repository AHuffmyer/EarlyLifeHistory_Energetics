...
```{r}
r_plot<-PRdf %>%
    ggplot(., aes(x = hpf, y = abs(R.nmol.org.min))) +
...
p_plot<-PRdf %>%
    ...
    scale_fill_manual(name="Lifestage", values=c("#DFC27D", "#80CDC1","#003C30"))+
    scale_color_manual(name="Lifestage", values=c("#DFC27D", "#80CDC1","#003C30"), labels=c("Embryo", "Larvae", "Metamorphosed \nPolyp"))+
    ylab(expression(bold(paste("Net Photosynthesis (nmol ", O[2], " larva"^-1, "min"^-1, ")")))) +
    scale_y_continuous(limits=c(-0.07, 0.1), labels = scales::number_format(accuracy = 0.01, decimal.mark = '.'))+
    theme_classic() + 
    theme(
      legend.position="none",
...
gp_plot<-PRdf %>%
    ...
    scale_fill_manual(name="Lifestage", values=c("#DFC27D", "#80CDC1","#003C30"))+
    scale_color_manual(name="Lifestage", values=c("#DFC27D", "#80CDC1","#003C30"), labels=c("Embryo", "Larvae", "Metamorphosed \nPolyp"))+
    ylab(expression(bold(paste("Gross Photosynthesis (nmol ", O[2], " larva"^-1, "min"^-1, ")")))) +
    scale_y_continuous(limits=c(-0.07, 0.1), labels = scales::number_format(accuracy = 0.01, decimal.mark = '.'))+
    theme_classic() + 
    theme(
      legend.position="none",
...
Plot panel of all figures  
```{r}
full_fig<-plot_grid(r_plot, gp_plot, pr_plot, labels=c("A", "B", "C"), label_size=20, ncol=3, nrow=1, rel_heights= c(1,1,1), rel_widths = c(0.9,0.9,1.2), label_y=1, align="h")

ggsave(filename="Mcap2020/Figures/Respiration/respirometry_fig.png", plot=full_fig, dpi=300, width=18, height=6, units="in")
```

Plot P, R and PR ratio into one panel  
```{r}
stats<-PRdf%>%
  group_by(group, Lifestage, hpf)%>%
...
combined_plot<-PRdf %>%
    ...
    #add respiration
    geom_point(aes(fill=Lifestage, colour=Lifestage, group=group), pch = 21, alpha=0.4, size=4, position = position_jitterdodge(0.1)) + 
  ...
  geom_point(data=stats, aes(y=meanPR/20, colour=Lifestage, fill=Lifestage, group=group),  pch = 23, size=6, position = position_dodge(0.8)) +
     geom_errorbar(data=stats, aes(y=meanPR/20, x=hpf, ymin=ymin/20, ymax=ymax/20, colour=Lifestage, group=group), width=0.1, color="black", position=position_dodge(0.8)) +
...
combined_full<-plot_grid(combined_plot, legend, nrow=1, ncol=2, rel_widths=c(1, 0.3))
ggsave("Mcap2020/Figures/Respiration/Combined_Metrics_PR.png", combined_full, dpi=300, w=7, h=5, units="in")
```
...
```{r}
stats<-PRdf%>%
  dplyr::select(Lifestage, group, hpf, R.nmol.org.min, GP.nmol.org.min, ratio)%>%
  ...
  group_by(Lifestage, group, hpf, Metric)%>%
...
combined_plot_mean<-stats %>%
    ...
    geom_point(data = . %>% filter(Metric=="Respiration"), aes(fill=Lifestage, colour=Lifestage, group=group), pch = 21, alpha=0.4, size=6, position = position_dodge(0.8)) + 
  ...
  geom_point(data = . %>% filter(Metric=="P:R Ratio"), aes(y=mean/20, colour=Lifestage, fill=Lifestage, group=group),  pch = 23, size=6, position = position_dodge(0.8)) +
     geom_errorbar(data = . %>% filter(Metric=="P:R Ratio"), aes(y=mean/20, x=hpf, ymin=ymin/20, ymax=ymax/20, colour=Lifestage, group=group), width=0.1, color="black", position=position_dodge(0.8)) +
...
ggsave("Mcap2020/Figures/Respiration/Combined_Metrics_Means_PR.png", combined_full_mean, dpi=300, w=7, h=5, units="in")
```
