...
gene_matrix[, 2:63228] <- sapply(gene_matrix[, 2:63228], as.numeric)
...
Why is my metabolite matrix in a different format than gene expression? 

ERROR: Error in w * x : non-numeric argument to binary operator


```{r}
genes_shrink<-cov.shrink(gene_matrix) 
```

```{r}
metab_shrink<-cov.shrink(metabolites_matrix) 
```