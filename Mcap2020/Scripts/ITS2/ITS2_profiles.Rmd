...
Load data obtained from SymPortal analysis on URI HPC cluster. Load counts of ITS2 profiles for each sample.  
C profile = C31.C17d.C21.C21ac.C31a
D profile = D1.D4.D6.D1ab.D17d

```{r}
its2<-read.table(file="Mcap2020/Data/ITS2/mcap.profiles.absolute.abund_and_meta.txt", skip=6, sep="\t")
...
```{r}
metadata<-read.csv(file="Mcap2020/Data/ITS2/ITS2_metadata.csv")
metadata<-metadata%>%
...
```{r}
data$group <- factor(data$group, levels = c("Egg", "Embryo", "Larvae", "Metamorphosed Polyp", "Attached Recruit"))
...
plot4<-data%>%
  ...
  ggplot(., aes(x = as.factor(hpf), y = mean, fill=group, shape=Clade)) +
   ...
   geom_hline(yintercept=0.5, linetype="dashed", color = "darkgray")+
    ...
    scale_shape_manual(name="Genus", labels=c("Cladocopium", "Durusdinium"), values=c(16,5))+
...
ggsave(plot4, file="Mcap2020/Figures/ITS2/relabund.png", width=8, height=6, dpi=300)
```

Plot as a regression over development.  
```{r}
plot5<-data%>%
  spread(Clade, relabund)%>%
  mutate(ratio=C/D)%>%
  ...
  ggplot(., aes(x = hpf, y = ratio)) +
    geom_point(aes(group=group, colour=group), size=4, alpha=1, position = position_jitterdodge(0.3)) + 
...
ggsave(plot5, file="Mcap2020/Figures/ITS2/regression.png", width=6, height=6, dpi=300)
```
...
Analysis of C:D ratio over development.  
```{r}
model_data<-data%>%
  spread(Clade, relabund)%>%
  mutate(ratio=C/D)

model<-lm(ratio~as.numeric(hpf)*group, data=model_data)
summary(model)
anova(model)
```
There is a significant increase in C:D ratio over development.  

Calculate percent change in C over development.  

```{r}
data%>%
  spread(Clade, relabund)%>%
  summarise(perc_chg = (last(C) - first(C))/first(C))

data%>%
  spread(Clade, relabund)%>%
  summarise(perc_chg = (last(D) - first(D))/first(D))
...
Display summary of C and D proportions and ratios.  
...
model_data%>%
  group_by(hpf, group)%>%
  summarise(mean=mean(ratio), n=length(ratio), stdev=sd(ratio), sterr=stdev/sqrt(n))%>%
  kbl(caption="Descriptive statistics of C:D ratio")%>%
  kable_classic(full_width=FALSE, html_font="Arial")%>%
  row_spec(0, bold = TRUE) 
```

Plot C:D ratio over time.  
