...
From this, we are going to filter by adjusted p value <0.05, this is generated by the LRT test statistic. We had 9180 DEGs at P<0.05. We are also running at P<0.01 because we need a higher threshold due to the high number of DEGs. This was only reduced to 8686 with P<0.01. It was reduced further to P<0.001 with 8127 genes. 88.5% of genes were retained in the dataset at a stringent threshold. 
...
```{r}
dev.off()
pdf("Mcap2020/Figures/TagSeq/GenomeV3/DESeq/DEG_heatmap_development.pdf", width=12, height=12)
...
```
6330 genes were clustered with cluster size >100 
...
Plot these broken down into the main developmental patterns detected in PCA and heatmaps above (early, mid, and late development). Early development included 1-38 hpf; mid development included 65-163 hpf, and late development included 183-231 hpf. 

These are categorized in developmental stages as clusters that show a peak in the respective developmental periods. 
...
Plot averages of these clusters for each developmental period.  
...
cluster_plot2<-ggplot(cluster_data,
  ...
  ylab("Z-Score (VST normalized gene counts)")+
  xlab("")+
  geom_smooth(aes(group=genes), method = "loess", colour="darkgray", linewidth=0.1, se=FALSE)+
...
ggsave(cluster_plot2, filename="Mcap2020/Figures/TagSeq/GenomeV3/DESeq/cluster_plot_patterns.pdf", width=6, height=3.5)
```
...
#1845 genes in early development 
...
ggsave(plot=deg_FC, "Mcap2020/Figures/TagSeq/GenomeV3/DESeq/DEG_foldchange.pdf", height=4, width=4)
```
...
Clusters 6, 7, 16, 17, 19, 20, 21, and 25 do not have any enriched terms when analyzed alone. 
...
clusters 2 and 3 don't have enough enriched go terms to get past the rrvgo step - these are not included. 
```{r}
cluster_list

cluster_list<-c("1", "5", "11", "12", "14", "15", "18", "22")
...
```
There are 458 total enriched GO terms detected in all clusters.  

### Plot enrichment for each cluster

Clusters 1, 5, 11, 12, 14, 15, 18, and 22 have significantly enriched GO terms after rrvgo (7 clusters). 
...
ggsave(filename="Mcap2020/Figures/TagSeq/GenomeV3/DESeq/functional_enrichment/go_cluster1.pdf", go_plot_cluster1, width=10, height=10)
```
...
ggsave(filename="Mcap2020/Figures/TagSeq/GenomeV3/DESeq/functional_enrichment/go_cluster5.pdf", go_plot_cluster5, width=10, height=18)
```
...
ggsave(filename="Mcap2020/Figures/TagSeq/GenomeV3/DESeq/functional_enrichment/go_cluster11.pdf", go_plot_cluster11, width=10, height=26)
```
...
ggsave(filename="Mcap2020/Figures/TagSeq/GenomeV3/DESeq/functional_enrichment/go_cluster12.pdf", go_plot_cluster12, width=10, height=8)
```
...
ggsave(filename="Mcap2020/Figures/TagSeq/GenomeV3/DESeq/functional_enrichment/go_cluster14.pdf", go_plot_cluster14, width=10, height=10)
```
...
ggsave(filename="Mcap2020/Figures/TagSeq/GenomeV3/DESeq/functional_enrichment/go_cluster15.pdf", go_plot_cluster15, width=10, height=8)
```
...
ggsave(filename="Mcap2020/Figures/TagSeq/GenomeV3/DESeq/functional_enrichment/go_cluster18.pdf", go_plot_cluster18, width=10, height=10)
```

Cluster 22 (peaking in egg through 38 hpf embryo)
```{r}
go_plot_cluster22 <- cluster_go_results %>% 
  filter(cluster=="22")%>%
  dplyr::arrange(bh_adjust) %>%
  mutate(percHits=(numDEInCat/numInCat)*100)%>%
  ...
  theme(legend.position = "right",
        ...
        axis.title.x = element_text(size = 12, face = "bold"),
        plot.title = element_text(size = 14, face = "bold"), 
        strip.text.y.right=element_text(angle=0)); go_plot_cluster22

ggsave(filename="Mcap2020/Figures/TagSeq/GenomeV3/DESeq/functional_enrichment/go_cluster22.pdf", go_plot_cluster22, width=10, height=10)
```
...
```
There are 451 total enriched GO terms detected when analyzed by pattern groups.   
...
ggsave(filename="Mcap2020/Figures/TagSeq/GenomeV3/DESeq/functional_enrichment/go_early.pdf", go_plot_early, width=36, height=38)
...
write_csv(early_data, file="Mcap2020/Output/TagSeq/GenomeV3/DESeq/functional_enrichment/enrichment_results_early.csv")
```
...
ggsave(filename="Mcap2020/Figures/TagSeq/GenomeV3/DESeq/functional_enrichment/go_mid.pdf", go_plot_mid, width=12, height=10)
...
write_csv(mid_data, file="Mcap2020/Output/TagSeq/GenomeV3/DESeq/functional_enrichment/enrichment_results_mid.csv")
```
...
go_plot_late<-plot_grid(go_plot_lateA, go_plot_lateB, ncol=2, rel_widths=c(1,1))
 
ggsave(filename="Mcap2020/Figures/TagSeq/GenomeV3/DESeq/functional_enrichment/go_late.pdf", go_plot_late, width=24, height=16)
...
write_csv(late_data, file="Mcap2020/Output/TagSeq/GenomeV3/DESeq/functional_enrichment/enrichment_results_late.csv")
```
