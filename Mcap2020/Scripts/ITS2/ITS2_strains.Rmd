...
Output means for integration plot. 
...
rel_abund_data%>%
  ...
  mutate(ratio=`C31-C17d-C21-C21ac-C31a`/`D1-D4-D6-D1ab-D17d`)%>%
  group_by(lifestage)%>%
  summarise(mean_ratioCD=mean(ratio, na.rm=TRUE))%>%
  arrange(lifestage)%>%
  mutate(symbiont_CD_ratio=((mean_ratioCD-dplyr::first(.$mean_ratioCD))/dplyr::first(.$mean_ratioCD)))%>%
  select(lifestage, symbiont_CD_ratio)%>%
  write_csv("Mcap2020/Data/Integration/RelativeChange/ITS2_CD_relchange.csv")
```
...
Of the samples that are at >1% relative abundance, none are differential.

This isn't very informative, the most dominant taxa are not detected as differential.  

There is some small variation across time, but significant differences are only seen between eggs and some later larval stages. 

Plot the relative abundance of these two taxa.  
...
Plot across lifestages.  

```{r}
plot<-degs%>%
  mutate(sample=rownames(.))%>%
  pivot_longer(values_to="value", "names_to"="DIV", cols=c("C3", "1378_C"))

deg_plot <- plot%>%
  
  ggplot(aes(x=lifestage, y=value, color=DIV)) + 
  geom_boxplot(aes(fill=NULL), position=position_dodge(0.9))+
  geom_point(size=3, position=position_dodge(0.9)) + 
  scale_color_manual(values=c("black", "darkgray"))+
  theme_classic() + 
  theme(axis.text.x=element_text(angle=45, hjust=1, color="black")) + 
  theme(axis.title.x=element_blank())+
  theme(legend.position="right");deg_plot

ggsave(plot=deg_plot, file="Mcap2020/Figures/ITS2/deg_taxa.png", width=5, height=4)
ggsave(plot=deg_plot, file="Mcap2020/Figures/ITS2/deg_taxa.pdf", width=5, height=4)
```
