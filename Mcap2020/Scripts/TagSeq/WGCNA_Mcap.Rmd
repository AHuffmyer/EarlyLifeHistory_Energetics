...
```{r}
specific_genes<-rbind(carbanhyd_genes, glycsyn_genes, sugartr_genes, ammontr_genes, acetylcarb_genes, glutperox_genes, solutetr_genes, atpase_genes, glucosehomeo_genes)
```
...
specific_genes$lifestage<-metadata_ordered$code[match(specific_genes$Sample, metadata_ordered$sample_id)]
```
...
Plot H+ATPase
```{r}
lifestage_order<-c("Egg", "Embryo", "Larvae1", "Larvae2", "Larvae3", "Larvae4", "Larvae6", "Recruit2", "Recruit1plug", "Recruit2plug")

specific_genes$lifestage<-factor(specific_genes$lifestage, levels = lifestage_order)
  
atpase_plot<-specific_genes%>%
    group_by(Function, gene_id, lifestage)%>%
    summarise(mean=mean(vst), N=length(vst), sd=sd(vst), sem=sd/N)%>%
    filter(Function=="H+ ATPase")%>%
    ggplot(., aes(x = lifestage, y = mean, linetype=gene_id, group=gene_id)) +
    facet_wrap(~Function)+
...
ammonium_plot<-specific_genes%>%
    ...
    ggplot(., aes(x = lifestage, y = mean, group=gene_id, linetype=gene_id)) +
    facet_wrap(~Function)+
    geom_line(aes(group=gene_id), color="black", position = position_dodge(0.1)) +
    geom_point(aes(group=gene_id), size=2, position = position_dodge(0.1)) + 
  geom_errorbar(aes(group=gene_id, ymin=mean-sem, ymax=mean+sem), width=0.1, linetype="solid", color="black", position=position_dodge(0.1), size=0.5) + 
...
gene_plots<-plot_grid(anhydrase_plot, acetyl_plot, ammonium_plot, atpase_plot, glucose_plot, glutathione_plot, glycogen_plot, solute_plot, sugar_plot, ncol=5, nrow=2, align="h")
                      
ggsave(filename="Mcap2020/Figures/TagSeq/gene_expression_class.pdf", plot=gene_plots, dpi=300, width=16, height=6, units="in")
```
