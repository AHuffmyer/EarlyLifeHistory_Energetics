...
*Important! If geneInfo.csv is not found or already generated, run the WGCNA_Mcap_V3.Rmd in Scripts to generate. It is a large file that cannot be pushed to Git. 
...
This isn't useful right now with the number of terms that we have. 
```{r}
# module_vector<-c("12")
# ontologies<-c("BP")
# 
...
- How to do semantic similarity? 
- Why are goslim terms the same as the go terms output by goseq? How do I get true go slim terms or parent terms?

How to reduce set? Maybe subset for things that have "protein", "lipid", "carbohydrate", "metabolism", or other terms of interest? 
...
symbiosis<-c("regulation of cell communication", "transmembrane transport", "transmembrane", "organic substance transport", "antioxidant", "oxygen")
  ...
  GO.plot.sym <-  ggplot(go_results, aes(x = pattern, y = term)) + 
    geom_point(aes(size=bh_adjust, colour=pattern)) + 
    scale_size(name="Over rep. p-value", trans="reverse", range=c(3,6))+
    facet_grid(ParentTerm ~ ., scales = "free", space='free', labeller = label_wrap_gen(width = 15, multi_line = TRUE))+
    scale_colour_manual(values=c("lightgray", "darkgray", "black"), guide=FALSE)+
    xlab("Ontogenetic Pattern")+
...
pattern_plots<-pattern_expression%>%
  ...
  geom_point(aes(colour=group), size=6, alpha=0.8) +
