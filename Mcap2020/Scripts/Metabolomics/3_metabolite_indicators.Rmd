...
```{r}
tca<-c("Citrate", "Isocitrate", "Malate", "a-ketoglutarate", "Fumarate", "Succinate")
...
specific_metabolites%>%
  ...
  aov(log_norm_counts~HPF*compound, data=.)%>%
  summary()
```
...
glycolysis_plot<-glycolysis_df%>%
    ...
    facet_wrap(~Pathway)+
    geom_line(aes(group=compound, colour=compound), size=1) +
    geom_point(aes(group=compound, colour=compound), size=4) + 
    ...
    geom_text(x=1.5, y=0.8, label="p<0.001", color="black", size=4)+
    ylim(0,0.8) + 
    expand_limits(x = 18) + 
    theme_classic() + 
    theme(
...
glycolysis_plot_line<-glycolysis_df%>%
    ggplot(., aes(x = HPF, y = mean)) +
    facet_wrap(~Pathway)+
    geom_smooth(aes(group=compound, colour=compound), method="loess", alpha=0.2)+
    geom_dl(aes(label=glycolysis_df$compound), method=list("last.qp", cex=1, hjust=-0.15))+
    xlab("") + 
    ylab("log(normalized pool size)")+    
    scale_colour_brewer(palette="Dark2")+
    ylim(-0.05,0.8) + 
    geom_text(x=2, y=0.8, label="p<0.001", color="black", size=4)+
    expand_limits(x = 18) + 
    theme_classic() + 
    theme(
      legend.position="none",
...
gogat_plot<-gogat_df%>%
    ...
    xlab("") + 
    ylab("log(normalized pool size)")+
    geom_text(x=2, y=2.1, label="p<0.001", color="black", size=4)+
    #ylim(0,0.8) + 
    expand_limits(x = 18) + 
    theme_classic() + 
    theme(
...
gogat_plot_line<-gogat_df%>%
    ...
    scale_colour_brewer(palette="Dark2")+
    expand_limits(x = 18) + 
    geom_text(x=2, y=2.2, label="p<0.001", color="black", size=4)+
    theme_classic() + 
    theme(
      legend.position="none",
      axis.title=element_text(face="bold", size=14),
...
urea_plot<-urea_df%>%
    ...
    scale_colour_brewer(palette="Dark2")+
    #ylim(0,0.8) + 
    geom_text(x=2, y=1.4, label="p<0.001", color="black", size=4)+
    expand_limits(x = 18) + 
    theme_classic() + 
    theme(
      legend.position="none",
...
atp_plot<-atp_df%>%
    ...
    ylab("log(normalized pool size)")+
    scale_colour_brewer(palette="Dark2")+
    geom_text(x=2, y=0.5, label="p<0.001", color="black", size=4)+
    ylim(0,0.5) + 
    expand_limits(x = 18) + 
    theme_classic() + 
    theme(
...
antiox_plot<-antiox_df%>%
    ...
    ylab("log(normalized pool size)")+
    scale_colour_brewer(palette="Dark2")+
    geom_text(x=2, y=0.15, label="p<0.001", color="black", size=4)+
    ylim(0,0.15) + 
    expand_limits(x = 18) + 
    theme_classic() + 
    theme(
...
Run a one way anova to test variation in glycerol-3-phosphate (plot produced in script step 1 and generated to abundance plot) between 1 and 5 hpf.   
```{r}
metabolites%>%
  filter(compound=="Glycerol-3-phosphate")%>%
  filter(HPF %in% c("Egg (1 hpf)", "Embryo (5 hpf)"))%>%
...
tca_plot2<-tca_plot+theme(axis.text.x=element_blank(), axis.title.y=element_blank())
...
metabolite_plots<-plot_grid(glycolysis_plot2, tca_plot2, gogat_plot2, antiox_plot2, atp_plot2, cofactor_plot2, ncol=3, nrow=2, rel_heights = c(0.7,1))
                      
ggsave(filename="Mcap2020/Figures/Metabolomics/Pathways/metabolic_pathways.jpg", plot=metabolite_plots, dpi=300, width=24, height=10, units="in")
ggsave(filename="Mcap2020/Figures/Metabolomics/Pathways/metabolic_pathways.pdf", plot=metabolite_plots, dpi=300, width=24, height=10, units="in")
```
