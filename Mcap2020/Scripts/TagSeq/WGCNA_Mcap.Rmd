...
```{r}
go_mwu_me1<-geneInfo%>%
  select(gene_id, MM.1, p.MM.1)%>%
  mutate(MM.1 = if_else(p.MM.1>0.05, 0, MM.1))%>% #if a gene is associated with a module (p<0.05), keep the module membership score. If not, replace MM with a 0 per GOMWU instructions. 
  select(gene_id, MM.1)%>%
  write.csv(., "Mcap2020/Output/TagSeq/GOMWU/go_mwu_me1.csv", row.names=FALSE, quote = FALSE)
  
go_mwu_me2<-geneInfo%>%
  select(gene_id, MM.2, p.MM.2)%>%
  mutate(MM.2 = if_else(p.MM.2>0.05, 0, MM.2))%>%
  select(gene_id, MM.2)%>%
  write.csv(., "Mcap2020/Output/TagSeq/GOMWU/go_mwu_me2.csv", row.names=FALSE, quote = FALSE)
  
go_mwu_me3<-geneInfo%>%
  select(gene_id, MM.3, p.MM.3)%>%
  mutate(MM.3 = if_else(p.MM.3>0.05, 0, MM.3))%>%
  select(gene_id, MM.3)%>%
  write.csv(., "Mcap2020/Output/TagSeq/GOMWU/go_mwu_me3.csv", row.names=FALSE, quote = FALSE)

go_mwu_me5<-geneInfo%>%
  select(gene_id, MM.5, p.MM.5)%>%
  mutate(MM.5 = if_else(p.MM.5>0.05, 0, MM.5))%>%
  select(gene_id, MM.5)%>%
  write.csv(., "Mcap2020/Output/TagSeq/GOMWU/go_mwu_me5.csv", row.names=FALSE, quote = FALSE)

go_mwu_me6<-geneInfo%>%
  select(gene_id, MM.6, p.MM.6)%>%
  mutate(MM.6 = if_else(p.MM.6>0.05, 0, MM.6))%>%
  select(gene_id, MM.6)%>%
  write.csv(., "Mcap2020/Output/TagSeq/GOMWU/go_mwu_me6.csv", row.names=FALSE, quote = FALSE)
  
go_mwu_me7<-geneInfo%>%
  select(gene_id, MM.7, p.MM.7)%>%
  mutate(MM.7 = if_else(p.MM.7>0.05, 0, MM.7))%>%
  select(gene_id, MM.7)%>%
  write.csv(., "Mcap2020/Output/TagSeq/GOMWU/go_mwu_me7.csv", row.names=FALSE, quote = FALSE)
  
go_mwu_me8<-geneInfo%>%
  select(gene_id, MM.8, p.MM.8)%>%
  mutate(MM.8 = if_else(p.MM.8>0.05, 0, MM.8))%>%
  select(gene_id, MM.8)%>%
  write.csv(., "Mcap2020/Output/TagSeq/GOMWU/go_mwu_me8.csv", row.names=FALSE, quote = FALSE)

go_mwu_me9<-geneInfo%>%
  select(gene_id, MM.9, p.MM.9)%>%
  mutate(MM.9 = if_else(p.MM.9>0.05, 0, MM.9))%>%
  select(gene_id, MM.9)%>%
