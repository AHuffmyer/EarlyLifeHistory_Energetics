...
Filtering does not remove any metablites, all 185 metabolites are used in analysis.  
...
pdf("Mcap2020/Figures/Metabolomics/WGCNA/outliers.pdf")
...
```
NEED TO LEARN ABOUT POWERS FOR METABOLOMICS AS COMPARED TO GENE EXPRESSION. 

From this data, tt appears that our **soft thresholding power is 7**.  
...
Module identification is essentially cutting the branches off the tree in the dendrogram above. We like large modules, so we set the **minimum module size** relatively high, so we will set the minimum size at 3. 3 chosen for now to try out, but return to this for metabolomics specific numbers.  
...
We have 13 modules after merging with 85% similarity.   

Plot new tree with 13 modules.  
```{r}
...
We have 184 metabolites and 47 samples.  
...
```{r}
#names(data_filt) = gsub(pattern = "*.t1", replacement = "", x = names(datExpr))
...
Could export here to get list of metabolites? 


AH WORKING HERE TO PULL OUT LIST OF METABOLITES WITH MODULE MEMBERSHIP - do we even need anything below? 
