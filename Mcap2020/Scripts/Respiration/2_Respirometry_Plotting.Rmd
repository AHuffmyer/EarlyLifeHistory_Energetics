...
---
This script analyzes and plots data for M. capitata 2020 life history time series project. Plots are displayed in Plotting section. Results are provided in Analysis section. 
...
Calculate PR ratio.  
```{r}
fused_oxygen$ratio<-fused_oxygen$GP.nmol.org.min/(fused_oxygen$R.nmol.org.min*-1) #calculate P:R ratio 
```
...
Pplot1<-ggplot(data=PRdata, aes(x=dpf, y=P.nmol.org.min, colour=Lifestage, group=interaction(Lifestage, dpf))) +
  ...
  xlab("Days post fertilization")+
  ylab(expression(bold(paste("Net P (nmol ", O[2], " individual"^-1, "min"^-1, ")")))) +
  theme_classic() +
  theme(text = element_text(size = 18, color="black"),
        axis.text = element_text(size = 18, color="black"), 
        legend.position = "right",
...
GPplot1<-ggplot(data=PRdata, aes(x=dpf, y=GP.nmol.org.min, colour=Lifestage, group=interaction(Lifestage, dpf))) +
  ...
  scale_y_continuous(limits=c(-0.1, 0.3), labels = scales::number_format(accuracy = 0.01, decimal.mark = '.'))+
  xlab("Days post fertilization")+
  ylab(expression(bold(paste("Gross P (nmol ", O[2], " individual"^-1, "min"^-1, ")")))) +
  theme_classic() +
  theme(text = element_text(size = 18, color="black"),
        axis.text = element_text(size = 18, color="black"), 
        legend.position = "right",
