...
#went from 55 down to 46 with p value adjustment
...
EvE_pvalues<-EvE_pvalues%>%
  dplyr::select(compound, direction)%>%
...
EvE_bar<-EvE_VIP_plot %>%
  ...
  annotate("text", x = 40, y = -1.4, label = "Egg > Embryo", fontface =2, color="#8C510A", size=6) +
  #ggtitle(expression(bold("Eggs < Embryos"))) +
  theme_bw() + theme(panel.border = element_rect(linetype = "solid", color = "black"), panel.grid.major = element_blank(), #Makes background theme white
...
#went from 26 to 0 with p value adjustment
...
There are no significantly different metabolites between larvae and metamorphosed recruits.  
...
#went from 38 to 27 with p value adjustment
...
#went from 29 to 25 with p value adjustment
...
MvC_bar<-MvC_VIP_plot %>%
  ...
  annotate("text", x = 20, y = -1.6, label = "Metamorphosed Recruit > \nCalcifying Recruit", fontface =2, color="#003C30", size=6) +
  #ggtitle(expression(bold("Eggs < Embryos"))) +
  theme_bw() + theme(panel.border = element_rect(linetype = "solid", color = "black"), panel.grid.major = element_blank(), #Makes background theme white
...
VIP_bar_plots2<-plot_grid(LvC_bar, MvC_bar, ncol=2, nrow=1, rel_widths=c(1,1), rel_heights=c(1,1), align="vh")
...
## Plot venn diagram showing shared VIP metabolites for each life stage comparison.  
