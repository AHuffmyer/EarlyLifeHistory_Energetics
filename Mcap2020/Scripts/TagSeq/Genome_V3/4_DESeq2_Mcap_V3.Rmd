...
## Plot enrichment of up and down regulated genes at the Parent Term level  

Plot with up and down regulated with facet by comparison as a dot plot. 
...
Plot as a heatmap with blue for down regulated and red for up regulated. 

Create a dendrogram for the y axis. 

```{r}
test<-go_plot_parent_data%>%
  mutate(up_presence=if_else())
```
...
```{r}
go_plot_parent2a<-go_plot_parent_data%>%
  filter(batch=="1")%>%
  
  ggplot(aes(x=contrast, y=ParentTerm, colour=direction, fill=direction)) +
  geom_tile(aes(colour=direction))+
  #scale_size(name="Num. GO Terms", breaks=c(2,10,20,50,80), range = c(1, 5))+
  scale_colour_manual(values=c("blue2", "red2"))+
  scale_fill_manual(values=c("blue2", "red2"))+
  scale_x_discrete(limits=c("Embryo_5hpf", "Embryo_38hpf", "Embryo_65hpf", "Larvae_93hpf", "Larvae_163hpf", "Larvae_231hpf", "Polyp_231hpf", "Recruit_183hpf", "Recruit_231hpf"), labels=c("Embryo (5 hpf)", "Embryo (38 hpf)", "Embryo (65 hpf)", "Larvae (93 hpf)", "Larvae (163 hpf)", "Larvae (231 hpf)", "Polyp (231 hpf)", "Recruit (183 hpf)", "Recruit (231 hpf)"))+
  theme_classic()+
  xlab("")+
  ylab("Parent Term")+
  geom_vline(xintercept=2.5, linetype="dashed")+
...
go_plot_parent2b<-go_plot_parent_data%>%
  filter(batch=="2")%>%
  ...
  theme(legend.position = "right",
        ...
        axis.title.x = element_text(size = 12, face = "bold"),
        plot.title = element_text(size = 14, face = "bold"),
        axis.text.x = element_text(size=12, angle=45, vjust=1, hjust=1)); go_plot_parent2b


go_plot_parent_heatmap<-plot_grid(go_plot_parent2a, go_plot_parent2b, ncol=2, nrow=1, align="h", rel_widths=c(0.8,1))

ggsave(plot=go_plot_parent_heatmap, "Mcap2020/Figures/TagSeq/GenomeV3/DESeq/functional_enrichment/GO_parent_heatmap.png", height=10, width=16, limitsize=FALSE)

```

# Try detecting clusters from DEG results  
...
Filter for genes padj <0.001. For LRT, the p-value is the cut off associated with the test statistic. We have lots of genes, so choosing a more stringent cut off.  
...
# Subset to return genes with padj < 0.05 and fold change >1 abs
...
nrow(sigLRT_genes)
```
There are 4393 genes that change in some way across developmental stages at p<0.001 and at log2 FC > 2 (abs value).    

Perform clustering. 

Try first few genes for testing. 
```{r}
library(DEGreport)
...
RUN THIS ON RAVEN WITH HIGH NUMBER OF GENES 

Run with minimum 100 genes per cluster to detect the largest clusters. 
```{r}
...
ggsave(cluster_plot, filename="Mcap2020/Figures/TagSeq/GenomeV3/DESeq/cluster_plot.pdf", width=13, height=10)
```
...
```{r}
# Extract the cluster genes
group <- clusters$df %>%
          #dplyr::filter(cluster == 17)%>%
          rename(gene=genes)
```

We can perform functional enrichment on these patterns now.  
...
Only clusters 13 (peak in embryo 65 hpf to polyp 231 hpf), 8 (peak in egg to 38 hpf embryos), 2 (peak in larvae 93-231 hpf and polyps 231 hpf) and 1 (peak in polyp and larvae at 231 hpf) have enriched GO terms. The other clusters do not have any significantly enriched GO terms.     
...
Plot enrichment faceted by cluster

Cluster 1 (peak in polyp and larvae at 231 hpf)
```{r}
go_plot_cluster1 <- cluster_go_results %>% 
  ...
  theme(legend.position = "right",
        ...
        axis.title.y = element_text(size=12, face="bold"),
        axis.title.x = element_text(size = 12, face = "bold"),
        plot.title = element_text(size = 14, face = "bold")); go_plot_cluster1
```

Cluster 2 (peak in larvae 93-231 hpf and polyps 231 hpf)
```{r}
go_plot_cluster2 <- cluster_go_results %>% 
  filter(cluster==2)%>%
  dplyr::arrange(over_represented_pvalue) %>%
  mutate(percHits=(numDEInCat/numInCat)*100)%>%
  ...
  theme(legend.position = "right",
        ...
        axis.title.y = element_text(size=12, face="bold"),
        axis.title.x = element_text(size = 12, face = "bold"),
        plot.title = element_text(size = 14, face = "bold")); go_plot_cluster2
```

Cluster 8 (peak in egg to 38 hpf embryos)
```{r}
go_plot_cluster8 <- cluster_go_results %>% 
  filter(cluster==8)%>%
  dplyr::arrange(over_represented_pvalue) %>%
  mutate(percHits=(numDEInCat/numInCat)*100)%>%
  ...
  theme(legend.position = "right",
        ...
        axis.title.y = element_text(size=12, face="bold"),
        axis.title.x = element_text(size = 12, face = "bold"),
        plot.title = element_text(size = 14, face = "bold")); go_plot_cluster8
```

Cluster 13 (peak in embryo 65 hpf to polyp 231 hpf)
```{r}
go_plot_cluster13 <- cluster_go_results %>% 
  filter(cluster==13)%>%
  dplyr::arrange(over_represented_pvalue) %>%
  mutate(percHits=(numDEInCat/numInCat)*100)%>%
  ...
  theme(legend.position = "right",
        ...
        axis.title.y = element_text(size=12, face="bold"),
        axis.title.x = element_text(size = 12, face = "bold"),
        plot.title = element_text(size = 14, face = "bold")); go_plot_cluster13
```
